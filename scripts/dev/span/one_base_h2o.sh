# one_base_h2o.sh <order_file> <tag> <pairs> -- production qkv (X, V273 code, plain) vs V287 h2 under a forced order (Y).
# The order file applies to every kernel a build compiles, so the two variants come from two binaries: X from the
# V280 harness with only the qkv test (default arm = ops::sliding_project_qkv), Y from the same harness with v287.py
# applied, the default arm pointed at ops::sliding_project_qkv_h2 and SCHEDULER_MANUAL_ORDERING_PATH set. Jobs
# alternate X/Y, Y/X, ...; each job runs the one binary's qkv launches, so the pairing is across adjacent jobs.
set -u
ORD=${1:?order file}; TAG=${2:?tag}; PAIRS=${3:-8}
