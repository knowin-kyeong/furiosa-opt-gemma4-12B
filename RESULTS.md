...
| `V164_qkv_q_scatter_zero_index` | `V160` 브랜치(v157a 위) | q의 최종 store(head 레이아웃 `1 # 64`에서 `to_hbm_view`, V146이 이 경로를 scatter보다 +4k로 측정)를 **on-device zero index(kv_offset − kv_offset, `SubFxp` + Stash)로 `dma_scatter`**로 대체 | — | — | — | — | — | ? | 구현 중 — 인덱스 텐서는 `m![1]`이어야 하고(패딩 축은 dummy 라벨) DM 요소는 8 B 이상이어야 함 |
