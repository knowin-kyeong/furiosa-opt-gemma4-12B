...
| `V213_ffn_down_two_chunks` | `V209` | **V211을 적용한다.** down을 `m![H / 15 % 128, L / 7680]`(128 행그룹 × 2청크)로 옮기면 런이 3,840 B·전부 256 B 정렬 → V211대로 **−16k**. 대가 두 개와 그 해법: (a) 슬라이스당 x가 3,840 → 15,360 B로 4배 = HBM +5.9 MB → **V206에서 통한 ring-4 broadcast**로 사본 64개만 읽고 switch로 뿌리면 바이트는 그대로(1.97 MB)이고 switch만 ~1,920 정적. (b) V183/V184가 이 길에서 각각 +27.4k/+17.7k로 졌는데, V184가 hop을 고쳐 −9.4k를 회수했으므로 남은 손실은 x 경로였다 — (a)가 정확히 그것을 겨눈다 | — | — | — | — | 설계됨 | — | **설계됨 — 구현 전에 풀어야 할 형상 충돌 1개.** 슬라이스당 행이 **15**(1920/128)인데 pass B의 출력 패킹이 `transpose::<m![H % 60 = $rows / 4], ...>`라 **타일 행 수가 4의 배수여야 한다**(4행 transpose 규칙). 15는 4로 안 나뉜다. 검토한 우회: 120 행그룹 × 2청크 = 240 슬라이스는 **live 슬라이스가 2의 거듭제곱이어야 한다**는 규칙에 걸린다(RESULTS "패딩된 슬라이스 축" 항목). **유망한 우회는 겹치는 타일**이다 — 4@0, 4@4, 4@8, **4@11**(행 11을 한 번 더 계산·기록, 값이 같으므로 무해)로 15행을 4의 배수 타일 4개로 덮는다. 대가는 down 계산의 6.7%. 확인할 것: 컴파일러가 겹치는 `commit_view` 대상을 허용하는지 |
