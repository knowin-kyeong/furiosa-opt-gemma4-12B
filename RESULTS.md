...
| `V206_x_broadcast_ring_sweep` | `V200` | V205의 따름정리: 고정비의 정체는 대부분 **MainContext 작업이 실물에서 DMA 뒤에 숨지 않는 것**이다(정적 스케줄은 숨는다고 본다). 가장 큰 순수-이동 Main 항목은 x ring-32 broadcast로 qkv·ffn **각각 7,943 정적**. V158이 32를 고른 것은 첫 성공값일 뿐 스윕된 적이 없다. ring 32/16/8/4(클러스터당 사본 8/16/32/64)를 실측 A/B. 사본 축과 ring 축은 **서로 다른 축**이어야 한다(`m![Dummy256/k, Dummy256%k]`는 `Switch snoop bitmap disagrees`) → `Gf`(16)·`C`(64)를 사본 축으로 | 43,006 / 43,212 / 43,702 / 44,999 (r32/16/8/4) | — | 138,686 / 137,970 / 138,263 / 138,830 | — | 측정 중 | — | **측정 중.** 정적 switch 비용은 예측대로 8,927 → 2,207(qkv)로 떨어지지만 **makespan은 오히려 +2k** — 정적 모델은 Main이 숨는다고 보기 때문이다. **실측이 이기면 V205 모델(Main은 안 숨는다)이 확증되고 새 최적화 축이 열린다** |
