...
|---|---|---|---:|---:|---:|---:|:---:|:---:|:---:|
| `V236_sweep_attnout_tiles` | `V226` | **스윕 하네스 브랜치.** V209 채점 코드 + V226 하네스에 ffn 변형 두 개(`t4`, `d4`)를 달아 한 잡에서 3자 비교한다. 변형마다 첫 실행 출력을 대조하도록 ffn shim을 고쳤다(V235까지는 base만 검증했다) | — | — | — | — | — | — | 구현됨 |
| `V237_ffn_down_four_chunks` | `V236` | **down의 L 분할을 8청크 → 4청크.** V211이 잰 정렬 벌점(960 B 런, 6/8이 granule 중간에서 시작 → 5 granule로 4를 쓴다, 27%)을 1,920 B 런으로 없앤다: 정렬돼도 안 돼도 정확히 8 granule(128 + 1,920 = 2,048). fetch/useful 1.267 → 1.067. scale 런도 120 → 240 B. 대가는 live 슬라이스 128개(슬라이스당 열 2배)와 x 2배 — **V235가 ffn Main은 스트림 뒤에 숨는다고 확정했으므로 이 교환이 성립한다**. V200은 이 형상을 *길이* 곡선만으로 취소했는데 정렬 항이 그때는 측정되지 않았다 | — | — | — | — | — | — | 설계됨 → 구현됨 |
| `V238_ffn_down_four_tiles` | `V236` | **down 타일 5개(16/16/16/8/4) → 4개(16/16/16/12).** pass마다 4 KB LUT 재로드 838 + 타일 로드 고정비 ~550이므로 타일 하나가 정적 DMA ~1.4k, 그리고 Main·Sub pass 하나씩(V218 기준 실물 ~600) | — | — | — | — | — | — | 설계됨 → 구현됨 |
