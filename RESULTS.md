...
| `V217_qkv_channel_distributed_heads` | `V209` | **V215의 −6.5k를 실제로 가져오는 유일한 길이자 6.5 도달의 관문.** V215는 순환 행 매핑이 qkv 스트림을 465 → 514 B/cycle로 올린다는 것을 실측했지만, 그 매핑에서는 head h의 256행이 256슬라이스에 1행씩 흩어져 현행 ring-64 head gather가 ring-256(≈61k)이 된다. **그런데 흩어진 배치가 사실은 head 연산에 더 자연스럽다:** 슬라이스 p는 (cluster의 8개 head 각각의) **채널 p** 값을 갖는다. 그러면 (a) head RMSNorm의 제곱합은 switch가 아니라 **256슬라이스 inter-slice reduce**(VRU, 값싼 원시)가 되고, (b) RoPE의 cos/sin은 슬라이스당 스칼라 1개라 gather·왕복(V187이 6.5k로 값매김)이 사라지며, (c) q store와 k/v scatter도 채널 레이아웃에서 나간다. 남는 비용은 rotate_half가 채널 p ↔ p+128을 필요로 하는 것 하나이고, stride-128 교환은 `Broadcast1 { slice1: 128, slice0: 2 }`로 표현된다(`full/attention.rs:45`에 선례) | — | — | — | — | 설계됨 | — | **설계됨 — 이번 캠페인이 남긴 가장 큰 미착수 판돈.** 기대: 스트림 −6.5k + tail 일부(현 tail ≈12k 중 RoPE 왕복·head gather). qkv 101k → 90k대면 기하평균 Arena 기준 6.30 → **≈6.5**. 위험: `project_query`·`project_key_value`·세 head RMSNorm·RoPE·store/scatter를 한꺼번에 바꿔야 해서 한 브랜치 = 한 가설 원칙과 충돌한다 → **먼저 탐침으로 (a)만 확인할 것**(256슬라이스 inter-slice reduce가 컴파일되고 값이 맞는지) |
