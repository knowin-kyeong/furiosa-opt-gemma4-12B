...
| `V210_ffn_x_pieces_in_lanes` | `V209` | **가장 큰 판돈.** ffn pass A(`contract_up_gate_full`)는 **36,526 정적 Main**으로 프로젝트 최대 항목인데, x_trf의 Lane이 `m![1]`이라 **Contraction Engine 8차선 중 1개만** 쓰고, `Dummy2`(x hi/lo)가 fetch **시간축**에 있어 **모든 weight 패킷을 두 번 읽는다**(29.5 MB × 2 × 2행렬). Dummy2를 **Lane으로** 옮기면 두 차선이 한 번의 weight fetch를 나눠 쓴다 → pass A fetch 절반 기대. 대가: 출력에 Dummy2 부분합 축이 생기고 pass B가 그것까지 합쳐야 한다(pass B는 이미 열 reduce를 하므로 흡수 가능) | — | — | — | — | 설계·부분 구현 | — | **진행 중 — 핵심 가능성은 확인됐다.** `TrfTensor<..., m![Dummy2], m![H]>` + `fetch::<m![L % 30, H / 64], m![H % 64]>` + `contract_lane::<..., m![Dummy2, H / 16 % 4 # 8]>` 조합이 **contraction 단계에서 타입 검사를 통과한다**(브랜치 `V210_ffn_x_pieces_in_lanes`). 남은 일은 pass B(`up_gate_reduce_full_fns!`)가 `m![L % 30, Dummy2, H / 16]`을 받아 블록 스케일 적용 후 Dummy2까지 합치도록 고치는 것뿐이다. **V206이 세운 규칙(ffn Main은 실물 비용)대로면 정적 −18k는 실물 −35k급(ffn −11%)** |
