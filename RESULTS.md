...
|---|---|---|---:|---:|---:|---:|:---:|:---:|:---:|
| `V235_ffn_pieces_in_lane` | `V226` | **x의 두 f8 조각을 Lane으로 → LUT를 한 번만.** pass A는 **정확히 LUT 처리율 한계**다(≈12.5 elem/cycle): 슬라이스당 115,200 원소 = 9,216 cycle(V227의 dequant 실측 9,263)인데 `Dummy2`가 fetch 시간축에 있어 230,400 = 18,432(실측 18,263)를 문다. `Dummy2`를 TRF의 Lane으로 옮기면 weight fetch 한 번이 두 조각을 먹인다. **V210이 여기서 죽었다**(`StoVrf: packet part should not be broadcasted` — pass B가 블록 scale을 Dummy2 위로 **패킷 안에서** replay했다). 해법: replay를 **Time**으로 옮기고(pass A가 weight를 replay하는 바로 그 방식), 조각 합은 **별도 fold pass** 하나로 처리해 pass B를 손대지 않는다 | — | — | **138,178** (V209 138,830) | — | 잡 1개, 변형당 n=12, 짝 교대: 프로덕션 median **295,667** vs V235 **296,131 (+464, +0.16%)** | 3/3 **PASS** | **기각(중립) — 그러나 이것이 ffn 최대 발견이다.** 설계는 정확히 성공했다: **pass A 36,526 → 18,526 static Main(−18,000)**, fold pass +7,762, **MainContext 76,836 → 66,598**. 그런데 **실물은 움직이지 않았다.** ⇒ **ffn의 MainContext는 정말로 DMA 스트림 뒤에 숨는다**(정적 스케줄의 DmaEngine 94.2%가 맞았다). V206의 −16,351은 Main 절감이 아니라 그 변경의 **DMA 쪽 효과**였다는 뜻이다. **따라서 'ffn Main을 줄인다'는 가설군 전체가 닫힌다** — 남은 것은 DMA 바이트나 DMA 효율뿐이고 둘 다 이미 닫혀 있다. 부수: PeCore 4,970 → 12,617(명령 수는 같다) |
| `V234_ffn_wide_lut_packet` | `V226` | pass A의 fetch 패킷을 64 → 128 원소로 | — | — | — | — | — | — | **기각(분석) — 두 겹으로 닫힌다.** ① `mir: OutPacket must be 1 or 2 flits (32 bytes each), got 128 bytes`. ② 설령 통과해도 이득이 없다: book의 fetch 비용은 `Time × (Packet / read_size)`이고 read_size ≤ 32 B이므로 128 f4(64 B)는 2 read다 — 1,800 × 2 = 3,600으로 64 f4의 3,600 × 1과 **같다**. 이 계산이 pass A의 벽을 확정했다: fetch가 아니라 **LUT 12.5 elem/cycle**이다 → V235 |
| `V230_ffn_begin_interleaved` | `V226` | **book의 마지막 미사용 API.** up/gate pass A를 한 sequencer 연산으로 | — | — | — | — | — | — | **구조적으로 닫힘.** 프론트엔드는 타입 레벨 축 이름을 받지만 컴파일러가 만드는 축은 `interleave`로 이름이 바뀌고 contraction의 입력 검사가 거부한다: `declared [Gs_1=2, …]` vs `pipeline [interleave_1=2, …]`. **어떤 사용자 축도 이 이름을 가질 수 없다** — `Ident::new`가 "Ident must start with an uppercase ASCII letter"를 assert하므로 `interleave`는 `axes!`에 못 쓰고, `src/axes.rs`는 채점에서 되돌려진다. `begin_interleaved`는 Vector Engine의 pair 경로(`vector_intra_slice_unzip::<{ Ident::I }, …>`)로만 닿고 Ident는 device fn에서 못 쓴다 |
