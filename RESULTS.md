...
| `V213_ffn_down_two_chunks` | `V209` | **V211을 적용한다.** down을 `DownRowsByColumns2 = m![H / 15 % 128, L / 7680]`(128 행그룹 × 2청크)로 옮기면 weight 런이 3,840 B·전부 256 B 정렬(V211 −16k), block scale도 슬라이스당 120 B 세그먼트 60개 → **480 B 세그먼트 15개**(sub-256 2× 벌점 해소, −7k 추정). 슬라이스당 15행은 V214의 겹치는 4행 타일(0·4·8·**11**)로 덮는다(+7.6k 실측) | — | — | — | — | **컴파일러 SIGABRT** | — | **막힘(컴파일러) — 전부 구현했고 두 벽을 순서대로 넘었다.** (1) `m![H % 15]` 출력 버퍼는 슬라이스당 30 B라 `in-slice element extent is 30 B, not a multiple of the SRAM access width 8`로 거부된다. (2) `# 16`(32 B)·`# 20`(40 B) 어느 쪽으로 패딩해도 **컴파일러가 `signal: 6, SIGABRT`로 죽는다** — V202b(down 2·3분할 벌크 로드)와 **같은 크래시**다. 즉 down의 비-8청크 레이아웃은 툴체인 쪽 벽이다. **브랜치에 전부 남아 있다**(`down_tile2_fns!`, `broadcast_inv_s_down2`, `stage_packet_down2`, 겹치는 타일 호출). 다음 세션이 시도할 것: (a) `DownRows2`의 live 슬라이스가 2개뿐인 inter-slice reduce가 원인인지 분리(`1 # 2` 대신 다른 stride), (b) cargo-furiosa-opt 0.7.0으로 재시도, (c) 출력 패킹을 transpose 없이(f32 커밋 후 별도 cast) 재구성 |
