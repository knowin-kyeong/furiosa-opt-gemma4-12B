...
| `V219_pass_cost_calibration` | `V209` | 최소 vector pass N개(0/4/8/16)를 꼬리에 사슬로 붙여 pass 1개의 실물 비용을 직접 잰다. 결과는 버려지므로 커널은 정확도 PASS. `i8` = 같은 8개를 독립으로 | 43,006 / 43,285 / 43,285 / 44,426 / 43,330 — **정적으로는 4개든 8개든 +279로 공짜다**(스케줄러가 숨긴다고 본다) | — | — | — | 잡 2개(변형당 n=3) median: job1 n0 110,033 · n4 111,559 · n8 **99,963** · n16 104,958 · i8 107,671 / job2 n0 105,453 · n4 102,146 · n8 **97,227** · n16 101,102 · i8 101,979 | 전부 **PASS** | **판정 불가 — 설계 결함.** n0가 매 반복의 **첫 변형**이라 cold/warm 전이를 혼자 뒤집어쓴다(두 잡 모두 n0가 가장 느리다). 그래서 n0 기준 기울기는 음수가 나온다. 순서 편향이 없는 **n16 − n8만 쓰면 +4,995 / +3,875 = 8 pass에 평균 +4,435 ≈ pass당 554**로 원래 추정과 맞지만, **n8 − n4는 부호가 반대**(−11,596 / −4,919)라 확정할 수 없다. **⇒ V218에서 끌어낸 "pass 개수가 레버"는 아직 근거가 없고, V220·V221의 기대값도 미확정이다.** 재설계: 변형 순서를 균형화(라틴 방격)하고 노이즈가 1/6인 attn_out에서 잰다 |
