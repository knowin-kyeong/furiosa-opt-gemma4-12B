...
| `V190_attnout_x_on_chip` | `V182` | attn_out의 x 두 조각을 HBM에 굽지 않고 **칩 안에서** 배포: 만든 8슬라이스(`XSlices`)에서 `[Dummy2, Qs % 512]` 버퍼로 복사한 뒤 행 그룹 축으로 ring broadcast 1번 → store 2개 + 512디스크립터 2 MB 로드 소멸. x1 = `CustomBroadcast { ring_size: 256 }`, x2 = `Broadcast1 { slice1: 32, slice0: 8 }` | — | **24,781 (x1) / 24,062 (x2)** vs base 28,173 (**정적 −4.1k, −15%**) | — | — | 큐 ×2 | — | 정적으로 가장 큰 폭. 실물 대기 |
