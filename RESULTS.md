...
| `V219_pass_cost_calibration` | `V209` | **V218의 해석을 확정하는 교정 실험.** V218은 head RMSNorm 제거가 실물 −6,833인데 정적은 −1,263(5.4배)임을 보였지만, 명령 수로 나눈 초과분이 변형마다 **172 / 56 / 13**으로 제각각이라 "pass당 고정비" 모델은 아직 근거가 없다(제거들이 서로 비가산적이기도 하다: −6,833 + −11,566 vs −16,012). **그러므로 직접 잰다:** 슬라이스당 f32 스칼라 1개에 `AddF`만 하는 **최소 vector pass**를 N개(0/4/8/16) 꼬리에 붙이고 기울기를 본다. 결과가 버려지므로 커널은 **정확도 PASS를 유지**한다. 추가 arm `i8` = 같은 8개를 **의존 사슬 없이 독립**으로 → 직렬과 같으면 원인은 **in-order 발행 비용**, 독립이 훨씬 싸면 **의존 지연(노출)** | — | — | — | — | 설계됨 | — | **설계됨 — 다음에 실행. 이 기울기가 V220·V221의 예산을 정한다** |
