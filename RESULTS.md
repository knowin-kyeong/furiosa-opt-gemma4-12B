...
| E0 스케줄 census (브랜치 없음) | `V261` | **book의 두 DMA 규칙이 우리 스케줄에서 발동하는가.** ① "같은 bank 64 access" 규칙에 걸린 tensor-unit pass는 컴파일러가 DMA처럼 스케줄한다 — Main/Sub 노드가 `DmaEngine` 컨텍스트를 갖는지 센다. ② 레이아웃이 DMN에 고르게 안 나뉘면 명령이 쪼개진다 — 소스 줄당 DMA 명령 수를 센다 (`scripts/dev/census.py`) | — | — | — | — | V236 qkv · V258 attn/ffn 덤프: **①은 세 커널 모두 0건, ②도 0건** (2건 이상인 줄은 전부 같은 함수를 두 번 부른 것 — K/V 로드, norm weight). 대신 **attn contraction store = 2,040 static, 디스크립터 32개(개당 ~64)** vs 같은 7,680 B의 꼬리 store **648, 디스크립터 8개(개당 ~81)** | — | **두 규칙 모두 닫힌다. 그리고 E4(256 B 정렬 store)의 전제가 틀렸다** — book의 RMW ~50× 벌점은 이 크기에서 안 보이고, store 비용은 **정렬이 아니라 디스크립터 수**를 따른다. E4는 실행하지 않는다. attn_out의 다음 표적은 **contraction store의 디스크립터 수**(32개)다 |
