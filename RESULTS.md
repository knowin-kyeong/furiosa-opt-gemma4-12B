...
| `V257_attnout_one_piece` | `V252` | **x를 f8 한 조각으로.** attn_out의 tolerance는 셋 중 가장 느슨하고(atol **0.05** vs qkv 0.04, ffn 0.01) single-piece 질문은 더 빡빡한 두 커널에만 물어봤다(V137/V138 qkv, V142 ffn). 조각을 하나로 줄이면 contraction의 `Dummy2` 재생 축이 사라져 **weight 패킷을 두 번이 아니라 한 번 fetch**하고, split pass 하나와 x 바이트 절반이 없어진다 | — | — | — | — | 잡 2개: **48,655 vs 54,061 (−5,406, −10.0%)**, **43,977 vs 49,683 (−5,706, −11.5%)** | **PASS** — **그런데 이 PASS는 공허하다** | **채택 (사용자 판단) — 다만 근거를 반드시 남긴다.** 채점 fixture의 attn_out x는 `s.signs(ctx, "x", 1.0)`으로 **정확히 ±1**이다. 따라서 x·16 = ±16이 f8e4m3에 **정확히** 표현되고 **낮은 조각은 항상 0**이다 — 두 변형의 max\|Δ\|가 0.01562로 **바이트 단위 동일**했던 이유가 이것이다(그 값은 계산 오차가 아니라 출력 bf16 반올림 1 ulp다). **실제 attention 출력(value 행들의 볼록결합)에서는 한 조각이 ~3.6% 상대오차를 낸다.** ⇒ **Stage 1 전용이다. Stage 2로 그대로 가져가면 안 된다** |
