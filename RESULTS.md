...
|---|---|---|---:|---:|---:|---:|:---:|:---:|:---:|
| `V196_dmn_interleave` | `V194` | **캠페인 A의 본체 — V8을 실물로 재판정.** book: "DMN interleaving: alternate across 2 DMNs per cluster, else **50% loss**". 지금 레이아웃은 슬라이스당 57.6 KB(ffn up) **연속 블록**이라 소스 주소가 올라가는 동안 슬라이스가 1씩 증가 → 32블록 연속으로 같은 DMN에 쓴다. 슬라이스 배정을 `m![L / 30 % 8, L / 240 % 32]`(연속 행그룹이 슬라이스 32칸 간격 = DMN 8개 순회)로 바꾼다. **판정은 PASS-safe 추가 로드의 증분 A/B로** 한다(다운스트림 재작업 없이 DMA 속도만 분리): p0 = 현행 레이아웃 추가 로드, p1 = stride-32 인터리브, p2 = stride-128 | — | — | — | — | 설계됨 | — | **설계됨** — V8은 정적 makespan 불변(26,734 → 26,734)으로 기각됐고 실물 A/B는 한 번도 안 했다. 정적 모델은 슬라이스 배정 순서를 안 본다(V8 판정문) |
