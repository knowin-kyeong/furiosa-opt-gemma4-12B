...
const TESTS: &[Test] = &[
    // V225: finish the ring sweep. "" is ring 4 (production), r8 is 32 copies, r2 is 128.
    // Order rotates each repetition so no variant always eats the cold transition (V219).
    t("decoder_feedforward", "", 0.01),
    t("decoder_feedforward", "r8", 0.01),
    t("decoder_feedforward", "r2", 0.01),
    t("decoder_feedforward", "r8", 0.01),
    t("decoder_feedforward", "r2", 0.01),
    t("decoder_feedforward", "", 0.01),
    t("decoder_feedforward", "r2", 0.01),
...
async fn decoder_feedforward(ctx: &mut Context, fixture: &Fixture, variant: &str) -> Vec<(&'static str, Vec<f32>)> {
    ...
    match variant {
        "r8" => launch(ops::decoder_feedforward_r8, (
            ctx,
            &mut residual,
            &pre_ff_rms_weight,
            &up_weight_packed,
        ...
        "r2" => launch(ops::decoder_feedforward_r2, (
            ctx,
            &mut residual,
            &pre_ff_rms_weight,
            &up_weight_packed,
