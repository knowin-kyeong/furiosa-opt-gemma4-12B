...
const QKV_SWEEP: &[&str] = &["", "b", "q5", "kv5", "q9", "q9", "kv5", "q5", "b", "q5", "q9", "b", "kv5", ""];
...
async fn sliding_project_qkv(
    ...
    for (i, variant) in plan.order.iter().enumerate() {
        ...
        match *variant {
            ...
            "c0" => {
                launch(ops::probe_load_c0, (ctx, &q_weight)).await;
            }
            "c1" => {
                launch(ops::probe_load_c1, (ctx, &q_weight)).await;
            }
            "c01" => {
            ...
            "qx1" => {
                launch(ops::probe_load_qx1, (ctx, &q_weight)).await;
            }
            "qxh" => {
                launch(ops::probe_load_qxh, (ctx, &q_weight)).await;
            }
            "qs1" => {
                launch(ops::probe_load_qs1, (ctx, &q_weight)).await;
            }
            "qa5" => {
                launch(ops::probe_load_qa5, (ctx, &q_weight)).await;
            }
            "qa9" => {
                launch(ops::probe_load_qa9, (ctx, &q_weight)).await;
            }
            "b" => {
                launch(ops::probe_bal_b, (ctx, &q_weight, &k_weight, &v_weight)).await;
            }
            "q5" => {
                launch(ops::probe_bal_q5, (ctx, &q_weight, &k_weight, &v_weight)).await;
            }
            "kv5" => {
                launch(ops::probe_bal_kv5, (ctx, &q_weight, &k_weight, &v_weight)).await;
            }
            "q9" => {
                launch(ops::probe_bal_q9, (ctx, &q_weight, &k_weight, &v_weight)).await;
            }
