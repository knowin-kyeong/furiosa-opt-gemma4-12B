...
const TESTS: &[Test] = &[
    // V218: where is qkv's ~33k of non-stream time? a1 drops the three head RMSNorms,
    // a2 drops RoPE, a3 drops both. All FAIL by design; only the cycles matter.
    t("sliding_project_qkv", "", 0.04),
    t("sliding_project_qkv", "a1", 0.04),
    t("sliding_project_qkv", "a2", 0.04),
    t("sliding_project_qkv", "a3", 0.04),
    t("sliding_project_qkv", "", 0.04),
    t("sliding_project_qkv", "a1", 0.04),
    t("sliding_project_qkv", "a2", 0.04),
    t("sliding_project_qkv", "a3", 0.04),
...
async fn sliding_project_qkv(ctx: &mut Context, fixture: &Fixture, variant: &str) -> Vec<(&'static str, Vec<f32>)> {
    ...
    match variant {
        "a1" => launch(ops::sliding_project_qkv_a1, (
            ctx,
            &x,
            &q_weight,
            &k_weight,
        ...
        "a2" => launch(ops::sliding_project_qkv_a2, (
            ctx,
            &x,
            &q_weight,
            &k_weight,
        ...
        "a3" => launch(ops::sliding_project_qkv_a3, (
            ctx,
            &x,
            &q_weight,
            &k_weight,
