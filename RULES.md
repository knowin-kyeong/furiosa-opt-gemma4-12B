...
**이번 캠페인 슬롯:** `V326`(qkv 큐 전체 몫 균형 탐침 — **기각: q5 +1.2% · kv5 +0.7% · q9 −1.8%(9/12). 몫은 명령마다 맞춰야 하고 62.5%는 과하다 → qkv 클러스터 몫 계열 닫힘**) → `V327` 취소 · `V328`(ffn CustomBroadcast 정리 — **설계 단계 기각**: erf는 전역 스칼라 분배, inv_s는 클러스터별 값이라 switch 대안이 대량 pass·복제 로드·추가 동기화뿐) · `V329`(ffn h 재로드 사본 탐침) · V323 패키지 16잡 재측정(job 20380 `rngd rerun`) — **결과: a53 +2.0%(11/16 느림) · a67 +7.4%(16/16) → attn 클러스터 몫 계열 닫힘.** 6잡 −461(4/6)은 노이즈였다 — 클러스터 지연 계열 판정은 16잡으로.
