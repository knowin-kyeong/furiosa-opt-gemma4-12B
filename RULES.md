...
- **공식 리더보드 제출(`moa-submitter submit`)은 하루 10회다** (2026-09-10 사용자 갱신, 이전 5회). 확실한 개선(실측으로 확인된 것)만 올린다.
...
**세 커널이 같은 방향으로 함께 움직인다.** 이전 세션이 "attn_out draw 하나가 기하평균을 4% 흔든다"고
적은 것은 절반만 맞았다 — 흔들리는 것은 커널 하나가 아니라 **잡 전체의 머신 속도**이고, 진폭이 가장 큰
것이 qkv다(우리 하네스에서도 qkv warm stdev가 3.19%로 셋 중 최악이다).
...
**하루 10회 예산은 이렇게 쓴다: 검증된 개선이 있으면 1건 예치, 나머지는 최고 코드로 draw 수확.**
