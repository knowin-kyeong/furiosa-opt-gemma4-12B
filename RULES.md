...
- draw 대상은 **`V378_submit`**이다(ea270d1 = V368 + V369 + V371).
  - 돌던 V377 배치 1이 끝나면 `/root/drawswitch_v378.sh`가 `drawchain_follow.sh V378_submit 1 8`을 돈다.
  - 점수는 `/root/tk/v378scores.sh`(V377 draw는 `v377scores.sh`)로 모은다.
- **Stage 2 전에 되돌릴 목록에 추가:** V371(qkv x 한 f8 조각) → V377_submit의 V369 경로.

**판정 기준 변경 (사용자 승인 계획, 2026-09-13).**
- 리더보드는 draw 최고값이다. attn · qkv는 짝비교 중앙값에 더해 **arm별 pooled p10 · p05 · 최저와 job-bootstrap 구간**을 본다: `scripts/dev/paired_tails.py <TAG> <kernel> prod <arm>`. ffn은 σ가 작아 중앙값으로 충분하다.
- 근거는 V340이다. 비균등 타일은 짝비교 중앙값 −1.2%였지만 draw attn 최저가 38,347 → 42,620, p10이 40,405 → 42,822로 올랐고, Arena 로그로 다시 재면 pooled p10 +3.45% [+38, +2,436]다. 리더보드에는 손해였다.

**이번 라운드 (계획 `2-7-4197-cuddly-twilight.md`).**
- **V368_submit:** V367 + V313 대칭 attn. 검증 job 24443 25/25 ×2. draw 전에 V377로 대체했다.
- **V369 채택(정확):** qkv 입력 norm의 `÷rms`를 없앴다. q · k · v head norm이 스칼라를 상쇄한다.
  - 12/16 −0.86%, 잡별 최저 15/16, pooled p10 −0.93% [−1,477, −379].
  - `shared/xsw.rs` `weight_blocks_bf16`. 실제 모델에서도 성립한다.
- **V377_submit** = V368 + V369. 검증 job 24476 25/25 ×2.
- **V371 채택(Stage 1 전용):** qkv x를 f8 한 조각(`x⊙w·128`)으로 두고 Q/K/V contraction의 `Dummy2` 재생을 없앴다.
  - 15/16 −3.07%, pooled p10 −4.13% [−4,453, −2,049], 96/96 PASS.
- **V378_submit** = V377 + V371. 검증 job 24526 25/25 ×2.
- **V373 막힘:** ffn down 1024열 패딩 슬롯 로드(`L # 16384 / 1024`)의 `to_dm`에서 visa ICE가 난다(V296 부류). 정렬 청크 계열을 컴파일러 기준으로 닫는다. V374 · V376 취소, V375 보류.
- **V372 기각(Stage 1 전용):** attn 로컬 RMS.
  - 중앙값 +2.89%, pooled p10 +4.92% [+484, +3,658].
  - 병합 동기화를 없애면 운 좋게 짧던 대기도 사라진다. 첫 launch만 −5.2%였다.
- **V370 보류:** `tx`의 게이트 VRF를 만들 수 없다(부분 축은 다시 tile로 못 자르고, 타일 전체는 VRF 8,192 B를 넘는다).

**새로 확인한 사실.**
- std 0.6.0 상수: `VRF_BYTES` = 8,192, TRF 65,536.
  - ContractionWeight는 같은 타입 쌍뿐이다.
  - FpUnaryOp에 Sin · Cos · Exp · Log가 있다.
  - `vector_fxp_to_fp`(i32 → f32)가 있다.
- 1 PE = 64 슬라이스이고 PE마다 DMA 엔진이 하나다(book, `pe_count.rs`).
- 매크로 hygiene: 매크로 밖에서 넘긴 식별자는 매크로 안에서 선언한 함수 인자를 가리키지 못한다. arm 커널은 명시적 함수로 쓴다(V373 첫 빌드 실패).
- V377 첫 draw 셋은 6.4075 · 7.1633 · 6.6855이고 attn은 40,657 · 40,445다. V350~V367 draw 35회의 최저 42,242보다 아래로 돌아왔다.
...
**로드 모델** (V335 · V339 · V341 합산, span은 클러스터 0 기준)
1. 레이아웃이 같으면 클러스터 0은 늘 ~332 B/cycle로 로드한다. 클러스터 1은 클러스터 0이 로드하는 동안 ~171, 혼자일 때 ~332 B/cycle이다. 동기화 대기(= 클러스터 1 지연)는 이 차이에서 나온다.
2. 지연을 줄이는 방법은 클러스터 1의 **슬라이스당 바이트**를 줄이는 것뿐이다(V321 qa5 · qa9, V339 u60: 지연 14.3k → 5.8k). 행 묶음을 통째로 패딩하면(V323 · V324) 두 클러스터 모두 슬라이스당 바이트가 늘어 진다.
3. 그래도 로드 창 전체는 두 클러스터가 나눠 쓰는 대역폭에 묶인다. 모델상 최적 분할(c0 ≈ 66%)의 창이 ≈31.3k(sym 35.2k 대비 −11%)이고, u60 창 35.1k는 한 명령 sym 35.9k와 같다. **분할은 누가 기다리는지를 바꿀 뿐이다.**
4. **HBM 스택 분리는 틀렸다**(V335). 한 스택에 갇힌 클러스터는 로드가 1.68× 느려지고 지연은 그대로다. 지연의 원인은 HBM 채널 공유가 아니다.
5. **프로덕션 256 B 열 청크가 가장 빠른 읽기 패턴이다**(V341: c512 +5.9% · c1024 +7.0% · 행 교차 +1.1~1.4%).

**커널 결과.** V340 비균등 타일을 **채택**했다(32잡 24/32, −1.2%, p≈0.007). 모델 기대(−4k)보다 작은 이유는 정적 스케줄러다. 작은 c0 로드 둘과 store를 tile1 앞에 세워 tile1이 contraction0 끝(23.8k)에야 발행되고, 그래서 클러스터 0 사슬이 임계 경로가 된다.
V342 세 타일은 **기각**(ua +1.0%, ub +2.4%에 정확도 FAIL). V343 로컬 RMS(Stage 1 전용, 부정확)는 기대 −3%라 **보류**.

**같은 날 밤 결과 (V344~V347)**
- **V344 기각:** live 슬라이스 수(128 · 64)는 attn 청크 로드 속도를 바꾸지 않는다(±3% 노이즈). 슬라이스 0..127(DMN 절반)에 몰면 +41% → 처리율은 DMN(엔진) 수에 묶인다.
- **V345 진단:** 시간 분할 최적은 c0 63%(k63 −2.9% vs k60, 12/16)이고 실물 바닥은 ≈33.9k. 로드 전용 k60의 클러스터 1 지연은 4.3k뿐이다(커널 V340은 13.5k) → **커널 지연은 늦게 발행된 tile1 탓**.
- **V346 · V347 기각(정확도 FAIL):** tile1을 앞당기려고 패딩 DM 버퍼(`H % 120 # 240`)에 두 contraction을 커밋하고 owned `DmTensor::reshape`로 클러스터 타입을 오가자 **컴파일·정적 순서는 의도대로(tile1 11,979, store 뒤)인데 수치가 틀렸다**(V346 NaN, V347 max|Δ| 3.90). V340의 검증된 병합으로 되돌려도 틀리므로 원인은 패딩 DM 버퍼 커밋 또는 클러스터 reshape 쪽이다. 틀린 출력의 사이클이지만 v6 − ut ≈ −0.6k · v7 − ut ≈ +1.8k라 고쳐도 ~1% 안쪽 → 계열을 닫는다. 컴파일러 함정: 패딩 HBM 레이아웃의 오프셋 tile 쓰기는 `lir: incorrect buffer size`, reshape한 mutable view 변수는 `mir: expected a DmTensorViewMut`.

**다음 후보**
- ① **qkv 머리 지연 — 가장 큰 미개척 레버.** 실물 span(pod `/root/tk/jb_r1.log`, 생산 arm #1, 창 95.3k): **Q weight 로드가 9,483에야 발행된다.** 그 앞은 x 로드(0.1~1.6k) → **2.6k 공백**(정적 1,000~1,244의 tile view 디스크립터 명령 ~30개로 추정) → 입력 RMS weight 로드(4.2~5.6k) → **Q scale 로드**(6.6~8.9k — x 경로가 아니라 contraction 뒤 Q head norm이 쓴다) → DMA 1.2k(switch 추정). x staging은 26.8k에 끝나는데 contraction Q는 Q 로드 끝(35.1k)을 기다린다 ⇒ **Q 로드를 머리로 당기면 최대 ≈ −8k(qkv −8%, 기하평균 +2.9%)**. 정적 스케줄러는 작은 로드 셋을 Q 앞 DMA 직렬 체인에 두고, 채점에서는 순서 강제 env를 못 쓴다 → 작은 로드를 머리에서 빼는 구조(Q scale 로드를 늦게 소비되는 버퍼에 붙이기 등)나 tile view 수 줄이기부터 본다. 먼저 PE C 프로그램 덤프로 2.6k 공백의 정체를 확인할 것.
- ② 한 명령 안의 슬라이스당 비대칭(240행 묶음 = c0 144행 · c1 96행) — 미확인.
- ③ 클러스터 1 PE 트레이스 확보 — 지금 span은 클러스터 0만 보인다.

### 10.0t 2026-09-12 — 공식 1위 7.3136, 남은 최대 비용은 클러스터 1 지연, book이 알려 준 switch의 DMA 점유
...
**이번 캠페인 슬롯:** `V326`(qkv 큐 전체 몫 균형 탐침 — **기각: q5 +1.2% · kv5 +0.7% · q9 −1.8%(9/12). 몫은 명령마다 맞춰야 하고 62.5%는 과하다 → qkv 클러스터 몫 계열 닫힘**) → `V327` 취소 · `V328`(ffn CustomBroadcast 정리 — **설계 단계 기각**: erf는 전역 스칼라 분배, inv_s는 클러스터별 값이라 switch 대안이 대량 pass·복제 로드·추가 동기화뿐) · `V329`(ffn h 재로드 사본 탐침 — **설계 보류: 기대 순이득 ≈0**) · `V330`(ffn down 256 B 정렬 청크 — **설계 기각: 클러스터별 geglu inv_s가 청크 경계를 막는다**) · `V331`(ffn inv_s ring-256 제거 — **기각: 64 사본 + ring 4 +1.4%(10/12), 256 사본 직접 로드 +3.9%(12/12). 복제 디스크립터(≈21 cycle/개)가 CustomBroadcast의 DMA 점유보다 비싸다**) · `V333`(norm cast 병합 — **중립 +0.5%(6/12)**) · `V332`(x2 직접 commit — **LIR ICE `no entry found for key`로 막힘**) · `V334`(qkv 9/7 query-head 레이아웃 — **visa split ICE로 막힘 → q9 계열 닫힘**) · V323 패키지 16잡 재측정(job 20380 `rngd rerun`) — **결과: a53 +2.0%(11/16 느림) · a67 +7.4%(16/16) → attn 클러스터 몫 계열 닫힘.** 6잡 −461(4/6)은 노이즈였다 — 클러스터 지연 계열 판정은 16잡으로.
...
**draw 경제 (같은 날, pod draw 로그 pooled):** V313_submit n=38 평균 6.869, σ 2.82%, 최고 7.3136(+2.30σ) · V306 n=14 σ 2.58% · V273 n=20 σ 3.94% — 최고값은 가우시안 꼬리와 맞는다.
8.0은 현 코드에서 +5.8σ(불가능)이고, **코드 평균 +6%(≈7.28)면 8.0이 +3.5σ라 draw ~3,000회(4분 간격 10일)에 반반**이다 ⇒ draw는 계속 돌리되, 코드 평균을 몇 % 올리는 것이 8.0의 전제다.
