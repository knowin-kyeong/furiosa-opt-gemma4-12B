...
**다음 후보(미검증)**
- ① V340의 tile1을 tile0 직후에 발행하게 만들기. 소스 순서는 안 먹으니 그래프 의존성이나 한 명령 병합으로 한다.
- ② 한 명령 안의 슬라이스당 비대칭(240행 묶음 = c0 144행 · c1 96행). V323의 lowering 제약("패딩량이 묶음 크기의 배수")에 걸리는지부터 확인한다.
- ③ 클러스터 1 PE 트레이스 확보. 지금 span은 클러스터 0만 보여서 클러스터 1 사슬을 동기화 길이로 추정하고 있다.
- ④ vinxst 급(33.4k)은 이 모델의 바닥(창 31.3k + contraction · 꼬리)보다 낮다. 바이트/cycle 자체를 올리는 축이나 병합 구조를 없애는 축을 찾아야 한다.
