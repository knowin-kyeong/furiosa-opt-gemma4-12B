...
pub(crate) fn project_output_x1(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // value rows, which the value RMSNorm bounds by sqrt(Ds) = 16, so |x s| <= 256 < 448.
    let (x_hi, x_lo) = hi_lo_x(ctx, &xs, 16f32);
    // V190: the pieces stay on chip. They are copied into one [Dummy2, Qs % 512] buffer on the
    // eight slices that hold them, then one ring broadcast over the row-group axis fills all 32
    // groups of the cluster - instead of two HBM stores and 512 descriptors reading 2 MB back.
    let mut xs2: DmTensor<f8e4m3, Chip, Cluster, XSlices, m![Dummy2, Qs % 512]> = DmTensor::new();
    ctx.main
        .begin(x_hi.view())
        .fetch::<m![Qs / 32 % 16], m![Qs % 32]>()
        .collect::<m![Qs / 32 % 16], m![Qs % 32]>()
        .commit_trim::<m![Qs % 32]>()
        .commit_view(xs2.view_mut().tile::<m![Dummy2], 1, m![Dummy2 = 1 #{!} 2, Qs % 512]>(0));
    ctx.main
        .begin(x_lo.view())
        .fetch::<m![Qs / 32 % 16], m![Qs % 32]>()
        .collect::<m![Qs / 32 % 16], m![Qs % 32]>()
        .commit_trim::<m![Qs % 32]>()
        .commit_view(xs2.view_mut().tile::<m![Dummy2], 1, m![Dummy2 = 1 #{!} 2, Qs % 512]>(1));

    let src: DmTensorView<'_, f8e4m3, Chip, TwoClusters, XSlices, m![Dummy2, Qs % 512]> =
        unsafe { xs2.view().reshape() };
    let broadcast: DmTensor<f8e4m3, Chip, TwoClusters, m![Dummy256 / 8, Qs / 512], m![Dummy2, Qs % 512]> = ctx
        .main
        .begin(src)
        .fetch::<m![Dummy2, Qs / 32 % 16], m![Qs % 32]>()
        .switch::<m![Dummy256 / 8, Qs / 512], m![Dummy2, Qs / 32 % 16]>(SwitchConfig::CustomBroadcast { ring_size: 256 })
        .collect::<m![Dummy2, Qs / 32 % 16], m![Qs % 32]>()
        .commit_trim::<m![Qs % 32]>()
        .commit();
    let x: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![Dummy2, Qs % 512]> =
        unsafe { broadcast.reshape() };
    let x_trf: TrfTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![1], m![Dummy2, Qs % 512]> = ctx
        .sub
        .begin(x.view())
        .fetch::<m![Dummy2, Qs / 32 % 16], m![Qs % 32]>()
...
pub(crate) fn project_output_x2(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    let broadcast: DmTensor<f8e4m3, Chip, TwoClusters, m![Dummy256 / 8, Qs / 512], m![Dummy2, Qs % 512]> = ctx
        ...
        .begin(src)
        .fetch::<m![Dummy2, Qs / 32 % 16], m![Qs % 32]>()
        .switch::<m![Dummy256 / 8, Qs / 512], m![Dummy2, Qs / 32 % 16]>(SwitchConfig::Broadcast1 { slice1: 32, slice0: 8 })
        .collect::<m![Dummy2, Qs / 32 % 16], m![Qs % 32]>()
        .commit_trim::<m![Qs % 32]>()
        .commit();
    let x: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![Dummy2, Qs % 512]> =
