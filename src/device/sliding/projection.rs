...
pub(crate) fn project_output_m1(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // value rows, which the value RMSNorm bounds by sqrt(Ds) = 16, so |x s| <= 256 < 448.
    let (x_hi, x_lo) = hi_lo_x(ctx, &xs, 16f32);
    // m1: one DMA command for both pieces (see V199).
    let mut x2: DmTensor<f8e4m3, Chip, Cluster, XSlices, m![Dummy2, Qs % 512]> = DmTensor::new();
    ctx.main
        .begin(x_hi.view())
        .fetch::<m![Qs / 32 % 16], m![Qs % 32]>()
        .collect::<m![Qs / 32 % 16], m![Qs % 32]>()
        .commit_trim::<m![Qs % 32]>()
        .commit_view(x2.view_mut().tile::<m![Dummy2], 1, m![Dummy2 = 1 #{!} 2, Qs % 512]>(0));
    ctx.main
        .begin(x_lo.view())
        .fetch::<m![Qs / 32 % 16], m![Qs % 32]>()
        .collect::<m![Qs / 32 % 16], m![Qs % 32]>()
        .commit_trim::<m![Qs % 32]>()
        .commit_view(x2.view_mut().tile::<m![Dummy2], 1, m![Dummy2 = 1 #{!} 2, Qs % 512]>(1));
    let mut x2_hbm: HbmTensor<f8e4m3, Chip, m![Qs / 512, Dummy2, Qs % 512]> = HbmTensor::new();
