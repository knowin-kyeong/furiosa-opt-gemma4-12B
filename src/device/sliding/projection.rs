...
pub(crate) fn project_output(
    ...
    let tile0: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 8, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 8, m![H / 60, H % 60 = 8 # 60, Qs]>(0)
        .to_dm(&mut ctx.tdma);
    let tile1: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 8, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 8, m![H / 60, H % 60 = 8 # 60, Qs]>(8)
        .to_dm(&mut ctx.tdma);
    let tile2: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 8, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 8, m![H / 60, H % 60 = 8 # 60, Qs]>(16)
        .to_dm(&mut ctx.tdma);
    let tile3: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 8, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 8, m![H / 60, H % 60 = 8 # 60, Qs]>(24)
        .to_dm(&mut ctx.tdma);
    let tile4: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 8, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 8, m![H / 60, H % 60 = 8 # 60, Qs]>(32)
        .to_dm(&mut ctx.tdma);
    let tile5: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 8, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 8, m![H / 60, H % 60 = 8 # 60, Qs]>(40)
        .to_dm(&mut ctx.tdma);
    let tile6: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 8, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 8, m![H / 60, H % 60 = 8 # 60, Qs]>(48)
        .to_dm(&mut ctx.tdma);
    ...
    ctx.main
        ...
        .commit_view(contraction.view_mut().tile::<m![H % 60], 8, m![H % 60 = 8 #{!} 60]>(0));
    ctx.main
        .begin(tile1.view())
        .fetch::<m![H % 60 = 8, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 8, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        ...
        .transpose::<m![H % 60 = 8 / 4], m![H % 60 = 8 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 8 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 8, m![H % 60 = 8 #{!} 60]>(8));
    ctx.main
        .begin(tile2.view())
        .fetch::<m![H % 60 = 8, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 8, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        ...
        .transpose::<m![H % 60 = 8 / 4], m![H % 60 = 8 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 8 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 8, m![H % 60 = 8 #{!} 60]>(16));
    ctx.main
        .begin(tile3.view())
        .fetch::<m![H % 60 = 8, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 8, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        ...
        .transpose::<m![H % 60 = 8 / 4], m![H % 60 = 8 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 8 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 8, m![H % 60 = 8 #{!} 60]>(24));
    ctx.main
        .begin(tile4.view())
        .fetch::<m![H % 60 = 8, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 8, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        ...
        .transpose::<m![H % 60 = 8 / 4], m![H % 60 = 8 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 8 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 8, m![H % 60 = 8 #{!} 60]>(32));
    ctx.main
        .begin(tile5.view())
        .fetch::<m![H % 60 = 8, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 8, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        ...
        .transpose::<m![H % 60 = 8 / 4], m![H % 60 = 8 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 8 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 8, m![H % 60 = 8 #{!} 60]>(40));
    ctx.main
        .begin(tile6.view())
        .fetch::<m![H % 60 = 8, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 8, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        ...
        .transpose::<m![H % 60 = 8 / 4], m![H % 60 = 8 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 8 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 8, m![H % 60 = 8 #{!} 60]>(48));
    ctx.main
        .begin(tile7.view())
