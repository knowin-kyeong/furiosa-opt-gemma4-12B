...
pub(crate) fn project_output_t52(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // applied by the post-attention RMSNorm (rmsnorm::normalize_add_scaled_reduced), which
    // keeps its load out of the front of the DMA queue.
    let tile0: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 52, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 52, m![H / 60, H % 60 = 52 # 60, Qs]>(0)
        .to_dm(&mut ctx.tdma);
    let tile1: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 8, Qs % 512]> = weight
        .view()
    ...
    ctx.main
        .begin(tile0.view())
        .fetch::<m![H % 60 = 52, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 52, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 60 = 52, Qs / 64 % 8, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 60 = 52]>()
        .contract_lane::<m![H % 60 = 52], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_inter_slice_reduce::<HiddenRows, m![H % 60 = 52]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 60 = 52 / 4], m![H % 60 = 52 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 52 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 52, m![H % 60 = 52 #{!} 60]>(0));
    ctx.main
        .begin(tile1.view())
...
pub(crate) fn project_output_t44(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // applied by the post-attention RMSNorm (rmsnorm::normalize_add_scaled_reduced), which
    // keeps its load out of the front of the DMA queue.
    let tile0: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 44, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 44, m![H / 60, H % 60 = 44 # 60, Qs]>(0)
        .to_dm(&mut ctx.tdma);
    let tile1: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 16, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 16, m![H / 60, H % 60 = 16 # 60, Qs]>(44)
        .to_dm(&mut ctx.tdma);
    ...
    ctx.main
        .begin(tile0.view())
        .fetch::<m![H % 60 = 44, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 44, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 60 = 44, Qs / 64 % 8, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 60 = 44]>()
        .contract_lane::<m![H % 60 = 44], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_inter_slice_reduce::<HiddenRows, m![H % 60 = 44]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 60 = 44 / 4], m![H % 60 = 44 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 44 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 44, m![H % 60 = 44 #{!} 60]>(0));
    ctx.main
        .begin(tile1.view())
        ...
        .commit_view(contraction.view_mut().tile::<m![H % 60], 16, m![H % 60 = 16 #{!} 60]>(44));
...
pub(crate) fn project_output_t40(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // applied by the post-attention RMSNorm (rmsnorm::normalize_add_scaled_reduced), which
    // keeps its load out of the front of the DMA queue.
    let tile0: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 40, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 40, m![H / 60, H % 60 = 40 # 60, Qs]>(0)
        .to_dm(&mut ctx.tdma);
    let tile1: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 20, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 20, m![H / 60, H % 60 = 20 # 60, Qs]>(40)
        .to_dm(&mut ctx.tdma);
    ...
    ctx.main
        .begin(tile0.view())
        .fetch::<m![H % 60 = 40, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 40, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 60 = 40, Qs / 64 % 8, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 60 = 40]>()
        .contract_lane::<m![H % 60 = 40], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_inter_slice_reduce::<HiddenRows, m![H % 60 = 40]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 60 = 40 / 4], m![H % 60 = 40 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 40 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 40, m![H % 60 = 40 #{!} 60]>(0));
    ctx.main
        .begin(tile1.view())
        ...
        .commit_view(contraction.view_mut().tile::<m![H % 60], 20, m![H % 60 = 20 #{!} 60]>(40));
...
pub(crate) fn project_output_t56(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // applied by the post-attention RMSNorm (rmsnorm::normalize_add_scaled_reduced), which
    // keeps its load out of the front of the DMA queue.
    let tile0: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 56, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 56, m![H / 60, H % 60 = 56 # 60, Qs]>(0)
        .to_dm(&mut ctx.tdma);
    let tile1: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 4, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 4, m![H / 60, H % 60 = 4 # 60, Qs]>(56)
        .to_dm(&mut ctx.tdma);
    ...
    ctx.main
        .begin(tile0.view())
        .fetch::<m![H % 60 = 56, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 56, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 60 = 56, Qs / 64 % 8, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 60 = 56]>()
        .contract_lane::<m![H % 60 = 56], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_inter_slice_reduce::<HiddenRows, m![H % 60 = 56]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 60 = 56 / 4], m![H % 60 = 56 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 56 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 56, m![H % 60 = 56 #{!} 60]>(0));
    ctx.main
        .begin(tile1.view())
        .fetch::<m![H % 60 = 4, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 4, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 60 = 4, Qs / 64 % 8, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 60 = 4]>()
        .contract_lane::<m![H % 60 = 4], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_inter_slice_reduce::<HiddenRows, m![H % 60 = 4]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 60 = 4 / 4], m![H % 60 = 4 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 4 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 4, m![H % 60 = 4 #{!} 60]>(56));
...
pub(crate) fn project_output_t60(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // applied by the post-attention RMSNorm (rmsnorm::normalize_add_scaled_reduced), which
    // keeps its load out of the front of the DMA queue.
    let tile0: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 60, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 60, m![H / 60, H % 60 = 60 # 60, Qs]>(0)
        .to_dm(&mut ctx.tdma);
    // x as two f8 pieces of x * s (see shared/f8split.rs), made once on eight slices and staged
    // through HBM so that each slice loads its column chunk of both pieces with one descriptor;
    // the tiles then contract f8 x f8 with no lookup pass, and the post-attention RMSNorm
    ...
    ctx.main
        .begin(tile0.view())
        .fetch::<m![H % 60 = 60, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 60, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 60 = 60, Qs / 64 % 8, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 60 = 60]>()
        .contract_lane::<m![H % 60 = 60], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_inter_slice_reduce::<HiddenRows, m![H % 60 = 60]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 60 = 60 / 4], m![H % 60 = 60 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 60 % 4]>()
        .commit_view(contraction.view_mut());
    // Each cluster writes its half of the [H] vector to HBM; the caller loads it back in the
    // layout it needs. (Collecting the 32 row groups onto one slice first, to cut the 64
    // store descriptors to 2, costs as much in the switch as it saves: the live slices sit
    // eight apart, so the ring spans all 256 slices, 2,055 cycles for 458 saved on the store.)
