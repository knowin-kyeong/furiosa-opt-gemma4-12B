...
pub(crate) fn project_output_into_v156(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    out: &mut HbmTensor<bf16, Chip, m![H]>,
    ...
    // V156: 8 full copies of x2 per cluster (16 descriptors) and one ring-32 switch pass that
    // hands every slot of a ring the chunk it needs (the Qs / 512 axis moves from time to
    // slice) instead of 512 descriptors reading the same 8 KB.
    let x8: DmTensor<f8e4m3, Chip, TwoClusters, m![Dummy8, 1 # 32], m![Qs / 512, Dummy2, Qs % 512]> =
        x2_hbm.to_dm(&mut ctx.tdma);
    let x: DmTensor<f8e4m3, Chip, TwoClusters, m![Dummy8, Dummy256 / 64, Qs / 512], m![Dummy2, Qs % 512]> = ctx
        .main
        .begin(x8.view())
        .fetch::<m![Qs / 512, Dummy2], m![Qs % 512]>()
        .switch::<m![Dummy8, Dummy256 / 64, Qs / 512], m![Dummy2]>(SwitchConfig::CustomBroadcast { ring_size: 32 })
        .collect::<m![Dummy2, Qs / 32 % 16], m![Qs % 32]>()
        .commit_trim::<m![Qs % 32]>()
        .commit();
