...
pub(crate) fn project_output(
    ...
    // Eight real copies (one store per copy): the 64 readers of each chunk spread over the copies
    // instead of all reading the same bytes. The load runs at 0.293 utilisation without them.
