...
/// V304: `project_output` with the O-weight as one 120-row tile.
pub(crate) fn project_output_1t(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H / 120, H % 120 # 128]> {
    ...
    // applied by the post-attention RMSNorm (rmsnorm::normalize_add_scaled_reduced), which
    // keeps its load out of the front of the DMA queue.
    // V304: all 120 rows of each row group in one load (one DMA command, one contraction pass).
    let w: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns256, m![H % 120, Qs % 256]> = weight.to_dm(&mut ctx.tdma);
    ...
    let contraction: DmTensor<bf16, Chip, TwoClusters, HiddenRows256, m![H % 120]> = ctx
        .main
        .begin(w.view())
        .fetch::<m![H % 120, Qs / 64 % 4], m![Qs % 64]>()
        .collect::<m![H % 120, Qs / 64 % 4, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 120, Qs / 64 % 4], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 120]>()
        .contract_lane::<m![H % 120], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_inter_slice_reduce::<HiddenRows256, m![H % 120]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
