...
pub(crate) fn project_output(
    ...
    // 2 real copies, one store each (a copy axis alone replicates nothing, V50); the readers of
    // a chunk spread over the copies instead of sharing one address (this load runs at 0.293).
    let mut x2_hbm: HbmTensor<f8e4m3, Chip, m![Dummy256 / 128, Qs / 512, Dummy2, Qs % 512]> = HbmTensor::new();
    for copy in 0..2 {
        x_hi.view().to_hbm_view(
            &mut ctx.tdma,
            x2_hbm
                .view_mut()
                .tile::<m![Dummy256 / 128], 1, m![1 #{!} 2, Qs / 512, Dummy2, Qs % 512]>(copy)
                .tile::<m![Dummy2], 1, m![1 #{!} 2, Qs / 512, Dummy2 = 1 #{!} 2, Qs % 512]>(0),
        );
        x_lo.view().to_hbm_view(
            ...
            x2_hbm
                .view_mut()
                .tile::<m![Dummy256 / 128], 1, m![1 #{!} 2, Qs / 512, Dummy2, Qs % 512]>(copy)
                .tile::<m![Dummy2], 1, m![1 #{!} 2, Qs / 512, Dummy2 = 1 #{!} 2, Qs % 512]>(1),
        );
    }

    let x: DmTensor<f8e4m3, Chip, TwoClusters, m![Dummy256 / 16, Dummy256 / 128, Qs / 512], m![Dummy2, Qs % 512]> =
