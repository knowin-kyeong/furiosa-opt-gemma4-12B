...
pub(crate) fn project_output_t104(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    ctx.main
        .begin(tile0.view())
        .fetch::<m![H % 120 = 104, Qs / 64 % 4, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 120 = 104, Qs / 64 % 4, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 120 = 104, Qs / 64 % 4, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
    ...
    ctx.main
        .begin(tile1.view())
        .fetch::<m![H % 120 = 16, Qs / 64 % 4, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 120 = 16, Qs / 64 % 4, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 120 = 16, Qs / 64 % 4, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
...
pub(crate) fn project_output_t72(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    ctx.main
        .begin(tile0.view())
        .fetch::<m![H % 120 = 72, Qs / 64 % 4, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 120 = 72, Qs / 64 % 4, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 120 = 72, Qs / 64 % 4, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
    ...
    ctx.main
        .begin(tile1.view())
        .fetch::<m![H % 120 = 48, Qs / 64 % 4, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 120 = 48, Qs / 64 % 4, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 120 = 48, Qs / 64 % 4, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
...
pub(crate) fn project_output_t60(
    ...
    // applied by the post-attention RMSNorm (rmsnorm::normalize_add_scaled_reduced), which
    // keeps its load out of the front of the DMA queue.
    let tile0: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns256, m![H % 120 = 60, Qs % 256]> = weight
        .view()
        .tile::<m![H % 120], 60, m![H / 120, H % 120 = 60 # 120, Qs]>(0)
        .to_dm(&mut ctx.tdma);
    let tile1: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns256, m![H % 120 = 60, Qs % 256]> = weight
        .view()
    ...
    ctx.main
        .begin(tile0.view())
        .fetch::<m![H % 120 = 60, Qs / 64 % 4, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 120 = 60, Qs / 64 % 4, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 120 = 60, Qs / 64 % 4, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 120 = 60]>()
        .contract_lane::<m![H % 120 = 60], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_inter_slice_reduce::<HiddenRows256, m![H % 120 = 60]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 120 = 60 / 4], m![H % 120 = 60 % 4 # 16]>()
        .commit_trim::<m![H % 120 = 60 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 120], 60, m![H % 120 = 60 #{!} 120]>(0));
    ctx.main
        .begin(tile1.view())
        .fetch::<m![H % 120 = 60, Qs / 64 % 4, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 120 = 60, Qs / 64 % 4, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        ...
        .transpose::<m![H % 120 = 60 / 4], m![H % 120 = 60 % 4 # 16]>()
        .commit_trim::<m![H % 120 = 60 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 120], 60, m![H % 120 = 60 #{!} 120]>(60));
...
pub(crate) fn project_output_t120(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // applied by the post-attention RMSNorm (rmsnorm::normalize_add_scaled_reduced), which
    // keeps its load out of the front of the DMA queue.
    let tile0: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns256, m![H % 120 = 120, Qs % 256]> = weight
        .view()
        .tile::<m![H % 120], 120, m![H / 120, H % 120 = 120 # 120, Qs]>(0)
        .to_dm(&mut ctx.tdma);
    // x as two f8 pieces of x * s (see shared/f8split.rs), made once on eight slices and staged
    // through HBM so that each slice loads its column chunk of both pieces with one descriptor;
    // the tiles then contract f8 x f8 with no lookup pass, and the post-attention RMSNorm
    ...
    ctx.main
        .begin(tile0.view())
        .fetch::<m![H % 120 = 120, Qs / 64 % 4, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 120 = 120, Qs / 64 % 4, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 120 = 120, Qs / 64 % 4, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 120 = 120]>()
        .contract_lane::<m![H % 120 = 120], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_inter_slice_reduce::<HiddenRows256, m![H % 120 = 120]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 120 = 120 / 4], m![H % 120 = 120 % 4 # 16]>()
        .commit_trim::<m![H % 120 = 120 % 4]>()
...
pub(crate) fn project_output_c512(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
