...
/// V380: `project_output` with O-weight tiles 88/32 instead of 96/24 (constants only).
pub(crate) fn project_output_t88(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H / 120, H % 120 # 128]> {
...
/// V380: `project_output` with O-weight tiles 104/16 instead of 96/24 (constants only).
...
/// V380: `project_output` with O-weight tiles 72/48 instead of 96/24 (constants only).
