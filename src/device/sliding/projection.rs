...
pub(crate) fn project_output_a4(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // store descriptors to 2, costs as much in the switch as it saves: the live slices sit
    // eight apart, so the ring spans all 256 slices, 2,055 cycles for 458 saved on the store.)
    // a4: the 32 row groups of a cluster are collected onto one slice and stored as one run of
    // 1920 elements, instead of 64 descriptors of 120 B from the padded layout. The ring delivers
    // packet-major (V181), so the gathered buffer is [packet, group] and the packing pass reads it
    // back in H order.
    let gathered: DmTensor<bf16, Chip, TwoClusters, Slice, m![H % 60 / 12, H / 60 % 32, H % 60 % 12]> = ctx
        .main
        .begin(contraction.view())
        .fetch::<m![H % 60 / 12], m![H % 60 % 12 # 16]>()
        .switch::<Slice, m![H % 60 / 12, H / 60 % 32]>(SwitchConfig::Broadcast1 { slice1: 32, slice0: 8 })
        .collect::<m![H % 60 / 12, H / 60 % 32], m![H % 60 % 12 # 16]>()
        .commit_trim::<m![H % 60 % 12]>()
        .commit();
    let packed: DmTensor<bf16, Chip, TwoClusters, Slice, m![H % 1920]> = ctx
        .main
        .begin(gathered.view())
        .fetch::<m![H / 60 % 32, H % 60 / 12], m![H % 60 % 12 # 16]>()
        .collect::<m![H % 1920 / 12], m![H % 60 % 12 # 16]>()
        .commit_trim::<m![H % 60 % 12]>()
        .commit();
    let mut gathered_hbm: HbmTensor<bf16, Chip, m![H]> = HbmTensor::new();
