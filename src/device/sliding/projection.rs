...
pub(crate) fn project_output(
    ...
    let tile0: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 28, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 28, m![H / 60, H % 60 = 28 # 60, Qs]>(0)
        .to_dm(&mut ctx.tdma);
    let tile1: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 16, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 16, m![H / 60, H % 60 = 16 # 60, Qs]>(28)
        .to_dm(&mut ctx.tdma);
    let tile2: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 16, Qs % 512]> = weight
        .view()
    ...
    ctx.main
        .begin(tile0.view())
        .fetch::<m![H % 60 = 28, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 28, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        .contract_outer::<m![H % 60 = 28, Qs / 64 % 8, Dummy2], m![Qs % 64], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 60 = 28]>()
        .contract_lane::<m![H % 60 = 28], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_inter_slice_reduce::<HiddenRows, m![H % 60 = 28]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 60 = 28 / 4], m![H % 60 = 28 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 28 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 28, m![H % 60 = 28 #{!} 60]>(0));
    ctx.main
        .begin(tile1.view())
        ...
        .commit_view(contraction.view_mut().tile::<m![H % 60], 16, m![H % 60 = 16 #{!} 60]>(28));
    ctx.main
        .begin(tile2.view())
        .fetch::<m![H % 60 = 16, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 16, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
