...
pub(crate) fn project_output(
    ...
    let tile0: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 12, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 12, m![H / 60, H % 60 = 12 # 60, Qs]>(0)
        .to_dm(&mut ctx.tdma);
    let tile1: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 12, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 12, m![H / 60, H % 60 = 12 # 60, Qs]>(12)
        .to_dm(&mut ctx.tdma);
    let tile2: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 12, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 12, m![H / 60, H % 60 = 12 # 60, Qs]>(24)
        .to_dm(&mut ctx.tdma);
    let tile3: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 12, Qs % 512]> = weight
        .view()
        .tile::<m![H % 60], 12, m![H / 60, H % 60 = 12 # 60, Qs]>(36)
        .to_dm(&mut ctx.tdma);
    let tile4: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![H % 60 = 12, Qs % 512]> = weight
        .view()
    ...
    ctx.main
        ...
        .commit_view(contraction.view_mut().tile::<m![H % 60], 12, m![H % 60 = 12 #{!} 60]>(0));
    ctx.main
        .begin(tile1.view())
        .fetch::<m![H % 60 = 12, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 12, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        ...
        .transpose::<m![H % 60 = 12 / 4], m![H % 60 = 12 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 12 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 12, m![H % 60 = 12 #{!} 60]>(12));
    ctx.main
        .begin(tile2.view())
        .fetch::<m![H % 60 = 12, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 12, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        ...
        .transpose::<m![H % 60 = 12 / 4], m![H % 60 = 12 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 12 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 12, m![H % 60 = 12 #{!} 60]>(24));
    ctx.main
        .begin(tile3.view())
        .fetch::<m![H % 60 = 12, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 12, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        ...
        .transpose::<m![H % 60 = 12 / 4], m![H % 60 = 12 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 12 % 4]>()
        .commit_view(contraction.view_mut().tile::<m![H % 60], 12, m![H % 60 = 12 #{!} 60]>(36));
    ctx.main
        .begin(tile4.view())
        .fetch::<m![H % 60 = 12, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 12, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
