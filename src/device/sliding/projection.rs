...
pub(crate) fn project_output_c128(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // the tiles then contract f8 x f8 with no lookup pass, and the post-attention RMSNorm
    // absorbs s.
    // V197 probe: one more full load of the O weight, same bytes and same slice count as the
    // graded load, but broken into 128 B runs instead of 512 B. Kept alive by a VRF pass.
    let probe: DmTensor<f8e4m3, Chip, TwoClusters, m![H / 240 % 8, Qs / 128], m![H % 240, Qs % 128]> =
        weight.to_dm(&mut ctx.tdma);
    let _probe_vrf: VrfTensor<f32, Chip, TwoClusters, m![H / 240 % 8, Qs / 128], m![H % 240 = 1, Qs % 128]> = ctx
        .sub
        .begin(probe.view().tile::<m![H % 240], 1, m![H % 240 = 1 # 240, Qs % 128]>(0))
        .fetch::<m![H % 240 = 1, Qs / 32 % 4], m![Qs % 32]>()
        .fetch_cast::<f32>()
        .collect::<m![H % 240 = 1, Qs / 8 % 16], m![Qs % 8]>()
        .to_vrf();
...
pub(crate) fn project_output_c256(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // absorbs s.
    // V197 probe: one more full load of the O weight, same bytes and same slice count as the
    // graded load, but broken into 256 B runs instead of 512 B. Kept alive by a VRF pass.
    let probe: DmTensor<f8e4m3, Chip, TwoClusters, m![H / 120 % 16, Qs / 256], m![H % 120, Qs % 256]> =
        weight.to_dm(&mut ctx.tdma);
    let _probe_vrf: VrfTensor<f32, Chip, TwoClusters, m![H / 120 % 16, Qs / 256], m![H % 120 = 1, Qs % 256]> = ctx
        .sub
        .begin(probe.view().tile::<m![H % 120], 1, m![H % 120 = 1 # 120, Qs % 256]>(0))
        .fetch::<m![H % 120 = 1, Qs / 32 % 8], m![Qs % 32]>()
        .fetch_cast::<f32>()
        .collect::<m![H % 120 = 1, Qs / 8 % 32], m![Qs % 8]>()
        .to_vrf();
...
pub(crate) fn project_output_c1024(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // absorbs s.
    // V197 probe: one more full load of the O weight, same bytes and same slice count as the
    // graded load, but broken into 1024 B runs instead of 512 B. Kept alive by a VRF pass.
    let probe: DmTensor<f8e4m3, Chip, TwoClusters, m![H / 30 % 64, Qs / 1024], m![H % 30, Qs % 1024]> =
        weight.to_dm(&mut ctx.tdma);
    let _probe_vrf: VrfTensor<f32, Chip, TwoClusters, m![H / 30 % 64, Qs / 1024], m![H % 30 = 1, Qs % 1024]> = ctx
        .sub
        .begin(probe.view().tile::<m![H % 30], 1, m![H % 30 = 1 # 30, Qs % 1024]>(0))
        .fetch::<m![H % 30 = 1, Qs / 32 % 32], m![Qs % 32]>()
        .fetch_cast::<f32>()
        .collect::<m![H % 30 = 1, Qs / 8 % 128], m![Qs % 8]>()
        .to_vrf();
...
pub(crate) fn project_output_c2048(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H]> {
    ...
    // absorbs s.
    // V197 probe: one more full load of the O weight, same bytes and same slice count as the
    // graded load, but broken into 2048 B runs instead of 512 B. Kept alive by a VRF pass.
    let probe: DmTensor<f8e4m3, Chip, TwoClusters, m![H / 15 % 128, Qs / 2048], m![H % 15, Qs % 2048]> =
        weight.to_dm(&mut ctx.tdma);
    let _probe_vrf: VrfTensor<f32, Chip, TwoClusters, m![H / 15 % 128, Qs / 2048], m![H % 15 = 1, Qs % 2048]> = ctx
        .sub
        .begin(probe.view().tile::<m![H % 15], 1, m![H % 15 = 1 # 15, Qs % 2048]>(0))
        .fetch::<m![H % 15 = 1, Qs / 32 % 64], m![Qs % 32]>()
        .fetch_cast::<f32>()
        .collect::<m![H % 15 = 1, Qs / 8 % 256], m![Qs % 8]>()
        .to_vrf();
