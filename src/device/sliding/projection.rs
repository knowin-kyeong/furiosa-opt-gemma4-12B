...
use crate::Chip;
use crate::axes::{Ds, Dummy2, Dummy256, Dummy8, Gs, H, Ns, Ps, Qs};
...
pub(crate) fn project_output_into_v161(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    out: &mut HbmTensor<bf16, Chip, m![H]>,
    ...
    // V161: 8 copies per (cluster, chunk) - 128 descriptors instead of 512 - and one ring-32 switch
    // pass (4 row groups x 8 chunks) with the chunk axis kept innermost.
    let x8: DmTensor<f8e4m3, Chip, TwoClusters, m![Dummy8, 1 # 4, Qs / 512], m![Dummy2, Qs % 512]> =
        x2_hbm.to_dm(&mut ctx.tdma);
    let x: DmTensor<f8e4m3, Chip, TwoClusters, m![Dummy8, Dummy256 / 64, Qs / 512], m![Dummy2, Qs % 512]> = ctx
        .main
        .begin(x8.view())
        .fetch::<m![Dummy2, Qs / 32 % 16], m![Qs % 32]>()
        .switch::<m![Dummy8, Dummy256 / 64, Qs / 512], m![Dummy2, Qs / 32 % 16]>(SwitchConfig::CustomBroadcast { ring_size: 32 })
        .collect::<m![Dummy2, Qs / 32 % 16], m![Qs % 32]>()
        .commit_trim::<m![Qs % 32]>()
        .commit();
    let x: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![Dummy2, Qs % 512]> = unsafe { x.reshape() };
    let x_trf: TrfTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns, m![1], m![Dummy2, Qs % 512]> = ctx
        .sub
        .begin(x.view())
        .fetch::<m![Dummy2, Qs / 32 % 16], m![Qs % 32]>()
...
pub(crate) fn project_output_into_v162(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    out: &mut HbmTensor<bf16, Chip, m![H]>,
...
pub(crate) fn project_output_into_v173(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    out: &mut HbmTensor<bf16, Chip, m![H]>,
    ...
    // store descriptors to 2, costs as much in the switch as it saves: the live slices sit
    // eight apart, so the ring spans all 256 slices, 2,055 cycles for 458 saved on the store.)
    // v173 probe: tile 0's contraction a second time into a scratch buffer stored to HBM, so
    // that the excess over the pass's own time reads the DMA/fetch interference.
    let mut contraction2: DmTensor<bf16, Chip, TwoClusters, HiddenRows, m![H % 60]> = DmTensor::new();
    ctx.main
        .begin(tile0.view())
        .fetch::<m![H % 60 = 48, Qs / 64 % 8, Dummy2], m![Qs % 64]>()
        .collect::<m![H % 60 = 48, Qs / 64 % 8, Dummy2, Qs / 32 % 2], m![Qs % 32]>()
        ...
        .transpose::<m![H % 60 = 48 / 4], m![H % 60 = 48 % 4 # 16]>()
        .commit_trim::<m![H % 60 = 48 % 4]>()
        .commit_view(contraction2.view_mut().tile::<m![H % 60], 48, m![H % 60 = 48 #{!} 60]>(0));
    let mut scratch2: HbmTensor<bf16, Chip, m![H]> = HbmTensor::new();
    contraction2.view().to_hbm_view(&mut ctx.tdma, scratch2.view_mut());
    contraction.view().to_hbm_view(&mut ctx.tdma, out.view_mut());
}

pub(crate) fn project_output_into_v167(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    out: &mut HbmTensor<bf16, Chip, m![H]>,
    ...
    // store descriptors to 2, costs as much in the switch as it saves: the live slices sit
    // eight apart, so the ring spans all 256 slices, 2,055 cycles for 458 saved on the store.)
    // V167: gather the 32 row groups of each cluster onto one slice through the switch (the
    // live slices sit eight apart, so the ring spans 256) and store [H % 1920] with one
    // descriptor per cluster instead of 64 from the padded layout.
    let gathered: DmTensor<bf16, Chip, TwoClusters, Slice, m![H / 60 % 32, H % 60]> = ctx
        .main
        ...
        .commit();
    let gathered: DmTensor<bf16, Chip, TwoClusters, Slice, m![H % 1920]> = unsafe { gathered.reshape() };
    gathered.view().to_hbm_view(&mut ctx.tdma, out.view_mut());
}

pub(crate) fn project_output_into_v163(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    out: &mut HbmTensor<bf16, Chip, m![H]>,
