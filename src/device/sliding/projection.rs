...
/// ST harness copy.
pub(crate) fn project_output_st(
    ctx: &mut Context,
    x: HbmTensorView<'_, bf16, Chip, m![Qs]>,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
) -> HbmTensor<bf16, Chip, m![H / 120, H % 120 # 128]> {
    ...
    // applied by the post-attention RMSNorm (rmsnorm::normalize_add_scaled_reduced), which
    // keeps its load out of the front of the DMA queue.
    // ST: both tiles land in one buffer, so the first contraction waits for the whole buffer and the second
    // tile's load is listed (and issued) before it -- on hardware the two loads then run concurrently.
    let mut rows: DmTensor<f8e4m3, Chip, TwoClusters, HiddenRowsByColumns256, m![H % 120, Qs % 256]> = DmTensor::new();
    weight
        .view()
        .tile::<m![H % 120], 96, m![H / 120, H % 120 = 96 # 120, Qs]>(0)
        .to_dm_view(&mut ctx.tdma, rows.view_mut().tile::<m![H % 120], 96, m![H % 120 = 96 #{!} 120, Qs % 256]>(0));
    weight
        .view()
        .tile::<m![H % 120], 24, m![H / 120, H % 120 = 24 # 120, Qs]>(96)
        .to_dm_view(&mut ctx.tdma, rows.view_mut().tile::<m![H % 120], 24, m![H % 120 = 24 #{!} 120, Qs % 256]>(96));
    let tile0 = rows.view().tile::<m![H % 120], 96, m![H % 120 = 96 # 120, Qs % 256]>(0);
    let tile1 = rows.view().tile::<m![H % 120], 24, m![H % 120 = 24 # 120, Qs % 256]>(96);
