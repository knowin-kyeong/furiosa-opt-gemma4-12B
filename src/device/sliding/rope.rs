...
/// V387: `apply_rope_heads_cc_1s` with the staging copy passes dependent on k (store, sync and reload after the K projection).
pub(crate) fn apply_rope_heads_cc_lt<C: M, S: M>(
    ctx: &mut Context,
    q: &DmTensor<bf16, Chip, C, S, m![Gs, Ds]>,
    k: &DmTensor<bf16, Chip, C, S, m![Ds]>,
    rope_offset: &HbmTensor<i32, Chip, m![1]>,
