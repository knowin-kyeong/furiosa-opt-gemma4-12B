...
// ---------------------------------------------------------------------------------------------
/// V385 `oc`: `apply_rope_heads_cc` with the cos/sin rows computed on the head slices (see gen_v385.py).
pub(crate) fn apply_rope_heads_cc_oc<C: M, S: M>(
    ...
    let x1_vrf: VrfTensor<f32, Chip, C, S, m![Dummy2, Ds % 2, 1 # 8]> = ctx
        .sub
        .begin(x1.view())
    ...
    let x2_vrf: VrfTensor<f32, Chip, C, S, m![Dummy2, Ds % 4, 1 # 8]> = ctx
        .sub
        .begin(x2.view())
    ...
    let x3_vrf: VrfTensor<f32, Chip, C, S, m![Dummy2, Ds % 8, 1 # 8]> = ctx
        .sub
        .begin(x3.view())
    ...
    let x4_vrf: VrfTensor<f32, Chip, C, S, m![Dummy2, Ds % 16, 1 # 8]> = ctx
        .sub
        .begin(x4.view())
    ...
    let x5_vrf: VrfTensor<f32, Chip, C, S, m![Dummy2, Ds % 32, 1 # 8]> = ctx
        .sub
        .begin(x5.view())
    ...
    let x6_vrf: VrfTensor<f32, Chip, C, S, m![Dummy2, Ds % 64, 1 # 8]> = ctx
        .sub
        .begin(x6.view())
    ...
    let a_vrf: VrfTensor<f32, Chip, C, S, m![Ds % 128, 1 # 8]> = ctx
        ...
        .collect::<m![Ds % 128], m![1 # 8]>()
        .to_vrf();
    let r: DmTensor<f32, Chip, C, S, m![Ds % 128, 1 # 8]> = ctx
        .main
        .begin(n.view())
        .fetch::<m![Ds % 128], m![1 # 8]>()
        .collect::<m![Ds % 128], m![1 # 8]>()
...
/// V385 `od`: `apply_rope_heads_cc` with the cos/sin rows computed on the head slices (see gen_v385.py).
pub(crate) fn apply_rope_heads_cc_od<C: M, S: M>(
    ctx: &mut Context,
    q: &DmTensor<bf16, Chip, C, S, m![Gs, Ds]>,
    k: &DmTensor<bf16, Chip, C, S, m![Ds]>,
    rope_offset: &HbmTensor<i32, Chip, m![1]>,
    ...
    ctx
        ...
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul1), 0.0009765625f32)
        .vector_widen_pad::<m![1 # 8]>()
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![Ds / 4 % 32], m![Ds % 4 # 16]>()
...
/// V385 `oe`: `apply_rope_heads_cc` with the cos/sin rows computed on the head slices (see gen_v385.py).
pub(crate) fn apply_rope_heads_cc_oe<C: M, S: M>(
    ctx: &mut Context,
    q: &DmTensor<bf16, Chip, C, S, m![Gs, Ds]>,
    k: &DmTensor<bf16, Chip, C, S, m![Ds]>,
    rope_offset: &HbmTensor<i32, Chip, m![1]>,
