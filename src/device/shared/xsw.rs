...
/// V364: `normalize_blocks_f32` whose mean-square pass adds `gate * 0` (a query-weight VRF): the x path waits for the
/// query weight load, so the head loads are issued without waiting for x-path passes.
pub(crate) fn normalize_blocks_f32_gated(
    ctx: &mut Context,
    x: &DmTensor<bf16, Chip, XCl, XBlocks, m![H % 480]>,
    rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    gate: &VrfTensor<f32, Chip, XCl, XBlocks, m![H % 480]>,
) -> DmTensor<f32, Chip, XCl, XBlocks, m![H % 480]> {
    ...
    let mean_square: DmTensor<f32, Chip, XCl, XBlocks, m![1 # 8]> = ctx
        ...
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_split::<m![H / 4 % 120], m![H % 4]>()
        .vector_fp_ternary_with_mode(FpTernaryOp::FmaF, TernaryArgMode::Mode120, (gate, 0f32))
        .vector_stash()
        .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul0), Stash)
        .vector_intra_slice_reduce::<H, m![1], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        .vector_fp_div(H_F32)
