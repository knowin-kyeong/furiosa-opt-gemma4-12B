...
pub(crate) fn normalize_blocks_bf16_fused(
    ...
    ctx.main
        ...
        .vector_widen_concat::<m![H / 8 % 60], m![H % 8]>()
        .vector_final()
        // V333: the bf16 cast rides the normalization pass (stage_x_hi_lo_full_blocks_bf16 has no cast-only pass).
        .cast::<bf16, m![H % 8 # 16]>()
        .commit_trim::<m![H % 8]>()
        .commit()
}

pub(crate) fn stage_x_hi_lo_full_blocks_bf16(
    ctx: &mut Context,
    normalized: &DmTensor<bf16, Chip, XCl, XBlocks, m![H % 480]>,
    up_global_scale: &HbmTensor<f32, Chip, m![1]>,
    gate_global_scale: &HbmTensor<f32, Chip, m![1]>,
) -> (
    DmTensor<f8e4m3, Chip, XCl, XBlocks, m![Dummy2, H % 480]>,
    ...
    // V333: x arrives as bf16 from normalize_blocks_bf16_fused.
    let x = normalized;
