...
/// Z1/V274 harness: V273_submit's post-attention RMSNorm (V263 fused).
pub(crate) fn normalize_add_scaled_reduced_v273<Cluster: M>(
    ctx: &mut Context,
    x: &DmTensor<bf16, Chip, Cluster, ReducingSlices, m![H % 480]>,
    channel_scale: &HbmTensor<bf16, Chip, m![H]>,
    rms_weight: &HbmTensor<bf16, Chip, m![H]>,
...
/// Z1/V274: the same norm whose sqrt pass writes the rms VRF directly.
pub(crate) fn normalize_add_scaled_reduced_v273_z1<Cluster: M>(
    ctx: &mut Context,
    x: &DmTensor<bf16, Chip, Cluster, ReducingSlices, m![H % 480]>,
    channel_scale: &HbmTensor<bf16, Chip, m![H]>,
    rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    ...
    let rms_in: DmTensorView<'_, f32, Chip, Cluster, ReducingSlices, m![1 # 8]> =
    ...
    let rms_vrf: VrfTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]> = ctx
        ...
        .vector_fp_unary(FpUnaryOp::Sqrt)
        .vector_widen_pad::<m![1 # 8]>()
        // Z1: this pass writes the VRF itself; the DM commit and the Sub staging pass are gone.
        .vector_final()
        .to_vrf();
