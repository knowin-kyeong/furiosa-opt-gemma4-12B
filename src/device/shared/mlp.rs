...
macro_rules! up_gate_reduce_full_fns {
    ($reduce:ident, $rows:literal) => {
        fn $reduce(
            ...
            // One block scale serves both f8 pieces, so the fetch replays it over the Dummy2 axis
            // the pass-A output now carries. Tile heights are halved against V204 because the VRF
            // operand is twice as long and the VRF holds 8 KB per slice.
