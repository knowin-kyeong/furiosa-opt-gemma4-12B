...
/// X6/V272: feedforward_v260 without the down global-scale pass (the post-FF RMSNorm absorbs it).
pub(crate) fn feedforward_x6(
    ctx: &mut Context,
    x2: &HbmTensor<f8e4m3, Chip, m![Dummy2, H]>,
    erf_scale: &HbmTensor<f32, Chip, m![1 # 8]>,
    out_scale: &HbmTensor<f32, Chip, m![1 # 8]>,
    ...
    let down_hbm: HbmTensor<bf16, Chip, m![H]> = unsafe { down_hbm.reshape() };
    let down = rmsnorm::load_reducing::<Cluster>(ctx, &down_hbm);
    // X6/V272: no global-scale pass. The post-FF RMSNorm is the only consumer and it is scale-invariant for a
    // positive per-tensor scale (the scale only rescales eps), so the multiply, its load and its VRF staging
    // are dropped from the kernel tail.
    let _ = down_global_scale;
    down
}
