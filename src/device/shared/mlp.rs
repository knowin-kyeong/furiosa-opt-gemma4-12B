...
pub(crate) fn feedforward(
    ...
    reduce_up_gate_rows_12(ctx, &up_partials, &up_scale, 32, &mut up);
    reduce_up_gate_rows_12(ctx, &gate_partials, &gate_scale, 32, &mut gate);
    reduce_up_gate_rows_12(ctx, &up_partials, &up_scale, 44, &mut up);
    reduce_up_gate_rows_12(ctx, &gate_partials, &gate_scale, 44, &mut gate);
