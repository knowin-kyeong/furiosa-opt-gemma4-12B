...
use crate::Chip;
use crate::axes::{C, Dummy2, Dummy256, Dummy8, H, L, Gs};
...
// V293: the ffn body taking x and the geglu scalars already replicated on chip.
pub(crate) fn feedforward_v181_sw(
    ctx: &mut Context,
    x: DmTensor<f8e4m3, Chip, BothClusters, Replicated, m![Dummy2, H]>,
    erf_all: DmTensor<f32, Chip, BothClusters, Replicated, m![1 # 8]>,
    out_all: DmTensor<f32, Chip, BothClusters, Replicated, m![1 # 8]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    gate_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    down_weight_packed: &HbmTensor<f4e2m1, Chip, m![H, L]>,
    up_weight_scale: &HbmTensor<f8e4m3, Chip, m![L, H / 16]>,
    ...
    // stream and on hardware it does not. qkv is a different case (its Main is small next to its
    // stream, and its x2 region is re-read by every copy), so qkv keeps ring 32.
    // V293: x arrives replicated on chip by xsw::replicate_blocks.
    let x: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![Dummy2, H]> = unsafe { x.reshape() };
    let x_trf: TrfTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![1], m![Dummy2, H]> = ctx
        .sub
        .begin(x.view())
    ...
    let g = geglu_full_sw(ctx, up, gate, erf_all, out_all);
    let x = gather_pack_full(ctx, &g);
    let (x2_hbm, inv_s_hbm) = stage_geglu_hi_lo_hbm(ctx, &x);
    let inv_s_vrf = broadcast_inv_s_down(ctx, &inv_s_hbm);
...
fn geglu_full_sw(
    ...
    let erf_all: DmTensor<f32, Chip, UpGateClusters, UpGateRowsFull, m![1 # 8]> = unsafe { erf_all.reshape() };
    let out_all: DmTensor<f32, Chip, UpGateClusters, UpGateRowsFull, m![1 # 8]> = unsafe { out_all.reshape() };
    let erf_scale_vrf = stage_packet_full(ctx, &erf_all);
    let out_scale_vrf = stage_packet_full(ctx, &out_all);
...
// ---------------------------------------------------------------------------------------------
// V301 = V235 on the V293 path: x's two f8 pieces in Lane, so the table lookup runs once per weight element.
// ---------------------------------------------------------------------------------------------
fn contract_up_gate_full_lane(
    ctx: &mut Context,
    ...
    ctx.main
        ...
        .contract_packet::<m![H / 16 % 4]>()
        .contract_time::<m![L % 30, H / 64]>()
        // Sequential relocates Lane into OutTime after Time; Interleaved would put it in OutPacket
        // and run the eight-wide output bus at a quarter rate with only two active lanes.
        .contract_lane::<m![L % 30, H / 64, Dummy2], m![H / 16 % 4 # 8]>(LaneMode::Sequential)
        .commit_trim::<m![H / 16 % 4]>()
        .commit()
}

/// Sum the two f8 pieces away, restoring the layout pass B already reads (V235).
fn fold_x_pieces(
    ctx: &mut Context,
...
pub(crate) fn feedforward_v301(
    ...
    // V293: x arrives replicated on chip by xsw::replicate_blocks.
    let x: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![Dummy2, H]> = unsafe { x.reshape() };
    // V301: the two f8 pieces of x go to Lane, not Time, so one weight fetch (and one LUT lookup) feeds both.
...
// V306: out_scale folded into the tail multiply.
fn geglu_full_sw_noout(
    ctx: &mut Context,
    up: DmTensor<f32, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, 1 # 8]>,
    gate: DmTensor<f32, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, 1 # 8]>,
    erf_all: DmTensor<f32, Chip, BothClusters, Replicated, m![1 # 8]>,
...
pub(crate) fn feedforward_v306(
    ...
    // V306: the geglu output scale is applied in the tail multiply (next to down_global_scale): one scalar
    // switch, its config load and one StoVrf leave the TUC-queue chain that delays the down_scale load (12/12, -1.8%).
...
// Tile-plan screen: 16,16,16,12 (single store)
pub(crate) fn feedforward_vtp(
    ctx: &mut Context,
    x: DmTensor<f8e4m3, Chip, BothClusters, Replicated, m![Dummy2, H]>,
    erf_all: DmTensor<f32, Chip, BothClusters, Replicated, m![1 # 8]>,
    out_scale: DmTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]>,
...
// ---------------------------------------------------------------------------------------------
// V348 (f1d): the down tiles take x in the TRF Lane (one fetch and one lookup per weight element) and sum the two f8
// pieces inside the same pass A (vector epilogue), so no fold pass follows. V303/V315 put x in Lane with a separate
// fold pass per tile and came out neutral; without the fold passes it is -3,509 cycles (-1.27%), 16/16 paired jobs.
// ---------------------------------------------------------------------------------------------

macro_rules! down_tile_lane_folded_fns {
    ($contract:ident, $rows:literal) => {
        /// V348: down pass A with `Dummy2` in the TRF Lane and the pieces summed in the same pass, in the partials
        /// layout pass B reads.
        fn $contract(
            ctx: &mut Context,
            x_trf: &TrfTensor<f8e4m3, Chip, DownClusters, DownRowsByColumns, m![Dummy2], m![L % 1920]>,
            packed: &DmTensor<f4e2m1, Chip, DownClusters, DownRowsByColumns, m![H % 60 = $rows, L % 1920]>,
        ) -> DmTensor<f32, Chip, DownClusters, DownRowsByColumns, m![H % 60 = $rows, L / 16 % 120]> {
            let folded: DmTensor<f32, Chip, DownClusters, DownRowsByColumns, m![H % 60 = $rows, L / 64 % 30, L / 16 % 4]> = ctx
                .main
                .begin(packed.view())
                ...
                .contract_packet::<m![L / 16 % 4]>()
                .contract_time::<m![H % 60 = $rows, L / 64 % 30]>()
                .contract_lane::<m![H % 60 = $rows, L / 64 % 30, Dummy2], m![L / 16 % 4 # 8]>(LaneMode::Sequential)
                .vector_init()
                .vector_intra_slice_tag(TagMode::Zero)
                .vector_narrow_trim::<m![L / 16 % 4]>()
                .vector_intra_slice_reduce::<Dummy2, m![H % 60 = $rows, L / 64 % 30], m![L / 16 % 4]>(IntraSliceReduceOpF32::Add)
                .vector_widen_pad::<m![L / 16 % 4 # 8]>()
                .vector_final()
                .commit_trim::<m![L / 16 % 4]>()
                .commit();
            unsafe { folded.reshape() }
        }
    };
}
down_tile_lane_folded_fns!(contract_down_rows_16_lane_folded, 16);
down_tile_lane_folded_fns!(contract_down_rows_12_lane_folded, 12);

/// V348 (f1d): `feedforward_vtp` with the down x pieces in Lane and summed inside each down pass A (no fold pass).
pub(crate) fn feedforward_f1d(
    ctx: &mut Context,
    x: DmTensor<f8e4m3, Chip, BothClusters, Replicated, m![Dummy2, H]>,
    erf_all: DmTensor<f32, Chip, BothClusters, Replicated, m![1 # 8]>,
    out_scale: DmTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]>,
    ...
    // Each slice loads only its 1920-wide chunk of the geglu output (both f8 pieces) from HBM; the pieces go to Lane.
    let x: DmTensor<f8e4m3, Chip, DownClusters, DownRowsByColumns, m![Dummy2, L % 1920]> = x2_hbm.to_dm(&mut ctx.tdma);
...
// ---------------------------------------------------------------------------------------------
// V349: the geglu hi/lo pieces gathered into one buffer by two copy passes and staged with one store, as the head path
// already does (m1 in `stage_x_hi_lo_hbm`). Every store adds an ExplicitSync, and three syncs sit back to back after
// the geglu stores on hardware (10.3k, 4.0k, 1.5k in trim_r11). V201 measured this merge at -628 before the span
// tooling and before V293.
// ---------------------------------------------------------------------------------------------

/// `stage_geglu_hi_lo_hbm` with both pieces written by one store command.
fn stage_geglu_hi_lo_hbm_one_store(
    ctx: &mut Context,
    x: &DmTensor<bf16, Chip, UpGateClusters, UpGateRowsGathered, m![L % 480]>,
) -> (HbmTensor<f8e4m3, Chip, m![L / 1920, Dummy2, L % 1920]>, HbmTensor<f32, Chip, m![L / 7680, 1 # 8]>) {
    let m_local = max_square_gathered(ctx, x);
...
/// V366 (T1): `feedforward_fo` without the tail multiply pass; returns the reloaded down output and g.
pub(crate) fn feedforward_fo_t1(
    ctx: &mut Context,
    x: DmTensor<f8e4m3, Chip, BothClusters, Replicated, m![Dummy2, H]>,
    erf_all: DmTensor<f32, Chip, BothClusters, Replicated, m![1 # 8]>,
    out_scale: DmTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]>,
...
macro_rules! down_tile_fns_c {
    ($load:ident, $contract:ident, $reduce:ident, $rows:literal) => {
        fn $load(
            ctx: &mut Context,
            packed: &HbmTensor<f4e2m1, Chip, m![H, L]>,
            offset: usize,
        ) -> DmTensor<f4e2m1, Chip, DownClustersC, DownRowsByColumnsC, m![H % 60 = $rows, L % 1920]> {
            packed
                .view()
                .tile::<m![H % 60], $rows, m![H / 60, H % 60 = $rows # 60, L]>(offset)
                .to_dm(&mut ctx.tdma)
        ...
        fn $contract(
            ctx: &mut Context,
            x_trf: &TrfTensor<f8e4m3, Chip, DownClustersC, DownRowsByColumnsC, m![1], m![Dummy2, L % 1920]>,
            packed: &DmTensor<f4e2m1, Chip, DownClustersC, DownRowsByColumnsC, m![H % 60 = $rows, L % 1920]>,
        ) -> DmTensor<f32, Chip, DownClustersC, DownRowsByColumnsC, m![H % 60 = $rows, L / 16 % 120]> {
            ctx.main
                .begin(packed.view())
                .fetch::<m![H % 60 = $rows, L / 64 % 30, Dummy2], m![L % 64]>()
                .fetch_table_lookup::<f8e4m3>()
        ...
        fn $reduce(
            ctx: &mut Context,
            partials: &DmTensor<f32, Chip, DownClustersC, DownRowsByColumnsC, m![H % 60 = $rows, L / 16 % 120]>,
            scale_all: &DmTensor<f8e4m3, Chip, DownClustersC, DownRowsByColumnsC, m![H % 60, L / 16 % 120]>,
            inv_s_vrf: &VrfTensor<f32, Chip, DownClustersC, DownRowsByColumnsC, m![1 # 8]>,
            offset: usize,
            out: &mut DmTensor<bf16, Chip, DownClustersC, DownRowsC, m![H % 60]>,
        ) {
            let scale_vrf: VrfTensor<f32, Chip, DownClustersC, DownRowsByColumnsC, m![H % 60 = $rows, L / 16 % 120]> = ctx
                .sub
                .begin(scale_all.view().tile::<m![H % 60], $rows, m![H % 60 = $rows # 60, L / 16 % 120]>(offset))
                .fetch::<m![H % 60 = $rows], m![L / 16 % 120]>()
                .fetch_cast::<f32>()
            ...
            ctx.main
                ...
                .vector_intra_slice_reduce::<L, m![H % 60 = $rows], m![1 # 4]>(IntraSliceReduceOpF32::Add)
                .vector_widen_pad::<m![1 # 8]>()
                .vector_inter_slice_reduce::<DownRowsC, m![H % 60 = $rows]>(InterSliceReduceOpF32::Add)
                .vector_final()
                .cast::<bf16, m![1 # 16]>()
                .transpose::<m![H % 60 = $rows / 4], m![H % 60 = $rows % 4 # 16]>()
                .commit_trim::<m![H % 60 = $rows % 4]>()
...
down_tile_fns_c!(load_down_rows_16_c, contract_down_rows_16_c, reduce_down_rows_16_c, 16);
down_tile_fns_c!(load_down_rows_12_c, contract_down_rows_12_c, reduce_down_rows_12_c, 12);

macro_rules! down_tile_lane_folded_fns_c {
    ($contract:ident, $rows:literal) => {
        /// V348: down pass A with `Dummy2` in the TRF Lane and the pieces summed in the same pass, in the partials
        /// layout pass B reads.
        fn $contract(
            ctx: &mut Context,
            x_trf: &TrfTensor<f8e4m3, Chip, DownClustersC, DownRowsByColumnsC, m![Dummy2], m![L % 1920]>,
            packed: &DmTensor<f4e2m1, Chip, DownClustersC, DownRowsByColumnsC, m![H % 60 = $rows, L % 1920]>,
        ) -> DmTensor<f32, Chip, DownClustersC, DownRowsByColumnsC, m![H % 60 = $rows, L / 16 % 120]> {
            let folded: DmTensor<f32, Chip, DownClustersC, DownRowsByColumnsC, m![H % 60 = $rows, L / 64 % 30, L / 16 % 4]> = ctx
                .main
                .begin(packed.view())
                .fetch::<m![H % 60 = $rows, L / 64 % 30], m![L % 64]>()
                .fetch_table_lookup::<f8e4m3>()
...
down_tile_lane_folded_fns_c!(contract_down_rows_16_lane_folded_c, 16);
down_tile_lane_folded_fns_c!(contract_down_rows_12_lane_folded_c, 12);

/// V390: `stage_geglu_hi_lo_hbm_one_store` returning 1/s on chip instead of storing it.
fn stage_geglu_hi_lo_hbm_x2(
    ctx: &mut Context,
    x: &DmTensor<bf16, Chip, UpGateClusters, UpGateRowsGathered, m![L % 480]>,
) -> (HbmTensor<f8e4m3, Chip, m![L / 1920, Dummy2, L % 1920]>, DmTensor<f32, Chip, UpGateClusters, m![Dummy256], m![1 # 8]>) {
    let m_local = max_square_gathered(ctx, x);
    let m_every: DmTensor<f32, Chip, UpGateClusters, m![Dummy256], m![L / 480 % 16, 1 # 8]> = ctx
        .main
        .begin(m_local.view())
    ...
    // V390: 1/s stays on chip -- every slice of both clusters already holds its cluster's value.
    (x2_hbm, inv_s_all)
}

/// V390: `feedforward_fo_t1` with the down stage split by column halves. V366 (T1): `feedforward_fo` without the tail multiply pass; returns the reloaded down output and g.
pub(crate) fn feedforward_fo_t1_c(
    ctx: &mut Context,
    x: DmTensor<f8e4m3, Chip, BothClusters, Replicated, m![Dummy2, H]>,
    erf_all: DmTensor<f32, Chip, BothClusters, Replicated, m![1 # 8]>,
    out_scale: DmTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]>,
    ...
    let p = contract_down_rows_16_lane_folded_c(ctx, &x_trf_lane, &down0);
    reduce_down_rows_16_c(ctx, &p, &down_scale, &inv_s_vrf, 0, &mut down);
    let p = contract_down_rows_16_lane_folded_c(ctx, &x_trf_lane, &down1);
    reduce_down_rows_16_c(ctx, &p, &down_scale, &inv_s_vrf, 16, &mut down);
    let p = contract_down_rows_16_lane_folded_c(ctx, &x_trf_lane, &down2);
    reduce_down_rows_16_c(ctx, &p, &down_scale, &inv_s_vrf, 32, &mut down);
    let p = contract_down_rows_12_lane_folded_c(ctx, &x_trf_lane, &down3);
...
/// V390b `c2`: `feedforward_fo_t1_c` with cluster 0's partial kept on chip.
pub(crate) fn feedforward_fo_t1_c2(
    ctx: &mut Context,
    x: DmTensor<f8e4m3, Chip, BothClusters, Replicated, m![Dummy2, H]>,
    erf_all: DmTensor<f32, Chip, BothClusters, Replicated, m![1 # 8]>,
    out_scale: DmTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]>,
    ...
    down_weight_scale: &HbmTensor<f8e4m3, Chip, m![H, L / 16]>,
    down_global_scale: &HbmTensor<f32, Chip, m![1]>,
) -> (DmTensor<bf16, Chip, Cluster, ReducingSlices, m![H % 480]>, DmTensor<bf16, Chip, Cluster, ReducingSlices, m![H % 480]>, DmTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]>) {
    let up_w: DmTensor<f4e2m1, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H]> = up_weight_packed.to_dm(&mut ctx.tdma);
    let up_scale: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H / 16]> = up_weight_scale.to_dm(&mut ctx.tdma);
    let gate_w: DmTensor<f4e2m1, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H]> = gate_weight_packed.to_dm(&mut ctx.tdma);
    let gate_scale: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H / 16]> = gate_weight_scale.to_dm(&mut ctx.tdma);
...
macro_rules! down_tile_fns_g {
    ($load:ident, $contract:ident, $reduce:ident, $rows:literal) => {
        fn $load(
            ctx: &mut Context,
            packed: &HbmTensor<f4e2m1, Chip, m![H, L]>,
            offset: usize,
        ) -> DmTensor<f4e2m1, Chip, DownClustersG, DownRowsByColumnsC, m![H % 60 = $rows, L % 1920]> {
        ...
        fn $contract(
            ctx: &mut Context,
            x_trf: &TrfTensor<f8e4m3, Chip, DownClustersG, DownRowsByColumnsC, m![1], m![Dummy2, L % 1920]>,
            packed: &DmTensor<f4e2m1, Chip, DownClustersG, DownRowsByColumnsC, m![H % 60 = $rows, L % 1920]>,
        ) -> DmTensor<f32, Chip, DownClustersG, DownRowsByColumnsC, m![H % 60 = $rows, L / 16 % 120]> {
            ctx.main
                .begin(packed.view())
                .fetch::<m![H % 60 = $rows, L / 64 % 30, Dummy2], m![L % 64]>()
                .fetch_table_lookup::<f8e4m3>()
        ...
        fn $reduce(
            ctx: &mut Context,
            partials: &DmTensor<f32, Chip, DownClustersG, DownRowsByColumnsC, m![H % 60 = $rows, L / 16 % 120]>,
            scale_all: &DmTensor<f8e4m3, Chip, DownClustersG, DownRowsByColumnsC, m![H % 60, L / 16 % 120]>,
            inv_s_vrf: &VrfTensor<f32, Chip, DownClustersG, DownRowsByColumnsC, m![1 # 8]>,
            offset: usize,
            out: &mut DmTensor<bf16, Chip, DownClustersG, DownRowsC, m![H % 60]>,
        ) {
            let scale_vrf: VrfTensor<f32, Chip, DownClustersG, DownRowsByColumnsC, m![H % 60 = $rows, L / 16 % 120]> = ctx
                .sub
                .begin(scale_all.view().tile::<m![H % 60], $rows, m![H % 60 = $rows # 60, L / 16 % 120]>(offset))
                .fetch::<m![H % 60 = $rows], m![L / 16 % 120]>()
                .fetch_cast::<f32>()
...
down_tile_fns_g!(load_down_rows_16_g, contract_down_rows_16_g, reduce_down_rows_16_g, 16);
down_tile_fns_g!(load_down_rows_12_g, contract_down_rows_12_g, reduce_down_rows_12_g, 12);

macro_rules! down_tile_lane_folded_fns_g {
    ($contract:ident, $rows:literal) => {
        /// V348: down pass A with `Dummy2` in the TRF Lane and the pieces summed in the same pass, in the partials
        /// layout pass B reads.
        fn $contract(
            ctx: &mut Context,
            x_trf: &TrfTensor<f8e4m3, Chip, DownClustersG, DownRowsByColumnsC, m![Dummy2], m![L % 1920]>,
            packed: &DmTensor<f4e2m1, Chip, DownClustersG, DownRowsByColumnsC, m![H % 60 = $rows, L % 1920]>,
        ) -> DmTensor<f32, Chip, DownClustersG, DownRowsByColumnsC, m![H % 60 = $rows, L / 16 % 120]> {
            let folded: DmTensor<f32, Chip, DownClustersG, DownRowsByColumnsC, m![H % 60 = $rows, L / 64 % 30, L / 16 % 4]> = ctx
                .main
                .begin(packed.view())
                .fetch::<m![H % 60 = $rows, L / 64 % 30], m![L % 64]>()
                .fetch_table_lookup::<f8e4m3>()
...
down_tile_lane_folded_fns_g!(contract_down_rows_16_lane_folded_g, 16);
down_tile_lane_folded_fns_g!(contract_down_rows_12_lane_folded_g, 12);

/// V390c: `feedforward_fo_t1_c2` on `Gs` clusters with cluster 1's partial stored alone.
pub(crate) fn feedforward_fo_t1_c4(
    ctx: &mut Context,
    x: DmTensor<f8e4m3, Chip, BothClusters, Replicated, m![Dummy2, H]>,
    erf_all: DmTensor<f32, Chip, BothClusters, Replicated, m![1 # 8]>,
    out_scale: DmTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]>,
    ...
    let gate_w: DmTensor<f4e2m1, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H]> = gate_weight_packed.to_dm(&mut ctx.tdma);
    let gate_scale: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H / 16]> = gate_weight_scale.to_dm(&mut ctx.tdma);
    let down0 = load_down_rows_16_g(ctx, down_weight_packed, 0);
    ...
    let down1 = load_down_rows_16_g(ctx, down_weight_packed, 16);
    let down2 = load_down_rows_16_g(ctx, down_weight_packed, 32);
    let down3 = load_down_rows_12_g(ctx, down_weight_packed, 48);
    ...
    let p = contract_down_rows_16_lane_folded_g(ctx, &x_trf_lane, &down0);
    reduce_down_rows_16_g(ctx, &p, &down_scale, &inv_s_vrf, 0, &mut down);
    let p = contract_down_rows_16_lane_folded_g(ctx, &x_trf_lane, &down1);
    reduce_down_rows_16_g(ctx, &p, &down_scale, &inv_s_vrf, 16, &mut down);
    let p = contract_down_rows_16_lane_folded_g(ctx, &x_trf_lane, &down2);
    reduce_down_rows_16_g(ctx, &p, &down_scale, &inv_s_vrf, 32, &mut down);
    let p = contract_down_rows_12_lane_folded_g(ctx, &x_trf_lane, &down3);
    reduce_down_rows_12_g(ctx, &p, &down_scale, &inv_s_vrf, 48, &mut down);
