...
// ---------------------------------------------------------------------------------------------
// V305: down contraction over a tile VIEW of one down-weight buffer loaded by a single DMA command.
// ---------------------------------------------------------------------------------------------
macro_rules! down_tile_view_fns {
    ($contract:ident, $rows:literal) => {
        fn $contract(
            ctx: &mut Context,
            x_trf: &TrfTensor<f8e4m3, Chip, DownClusters, DownRowsByColumns, m![1], m![Dummy2, L % 1920]>,
            packed: DmTensorView<'_, f4e2m1, Chip, DownClusters, DownRowsByColumns, m![H % 60 = $rows # 60, L % 1920]>,
        ) -> DmTensor<f32, Chip, DownClusters, DownRowsByColumns, m![H % 60 = $rows, L / 16 % 120]> {
            ctx.main
                .begin(packed)
                .fetch::<m![H % 60 = $rows, L / 64 % 30, Dummy2], m![L % 64]>()
...
down_tile_view_fns!(contract_down_view_16, 16);
down_tile_view_fns!(contract_down_view_12, 12);

pub(crate) fn feedforward_v305(
    ctx: &mut Context,
    x: DmTensor<f8e4m3, Chip, BothClusters, Replicated, m![Dummy2, H]>,
    erf_all: DmTensor<f32, Chip, BothClusters, Replicated, m![1 # 8]>,
    out_all: DmTensor<f32, Chip, BothClusters, Replicated, m![1 # 8]>,
    ...
    let gate_w: DmTensor<f4e2m1, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H]> = gate_weight_packed.to_dm(&mut ctx.tdma);
    let gate_scale: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H / 16]> = gate_weight_scale.to_dm(&mut ctx.tdma);
    // V305: all 60 rows of each down row group in ONE load (the contractions read 16/16/16/12-row views of it).
    let down_all: DmTensor<f4e2m1, Chip, DownClusters, DownRowsByColumns, m![H % 60, L % 1920]> =
    ...
    let mut down_hbm: HbmTensor<bf16, Chip, m![H / 60, H % 60]> = HbmTensor::new();
    let mut down: DmTensor<bf16, Chip, DownClusters, DownRows, m![H % 60]> = DmTensor::new();
    let p = contract_down_view_16(ctx, &x_trf, down_all.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 60, L % 1920]>(0));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 0, &mut down);
    let p = contract_down_view_16(ctx, &x_trf, down_all.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 60, L % 1920]>(16));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 16, &mut down);
    down.view()
        .tile::<m![H % 60], 30, m![H % 60 = 30 # 60]>(0)
        .to_hbm_view(
    ...
    let p = contract_down_view_16(ctx, &x_trf, down_all.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 60, L % 1920]>(32));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 32, &mut down);
    let p = contract_down_view_12(ctx, &x_trf, down_all.view().tile::<m![H % 60], 12, m![H % 60 = 12 # 60, L % 1920]>(48));
    reduce_down_rows_12(ctx, &p, &down_scale, &inv_s_vrf, 48, &mut down);
