...
pub(crate) fn stage_x_hi_lo_hbm(
    ...
    HbmTensor<f8e4m3, Chip, m![Dummy256 / 64, H / 1920, Dummy2, H % 1920]>,
    ...
    // 4 real copies, one store each (V50: a copy axis alone replicates nothing).
    let mut x2_hbm: HbmTensor<f8e4m3, Chip, m![Dummy256 / 64, H / 1920, Dummy2, H % 1920]> = HbmTensor::new();
    for copy in 0..4 {
        x_hi.view().to_hbm_view(
            &mut ctx.tdma,
            x2_hbm
                .view_mut()
                .tile::<m![Dummy256 / 64], 1, m![1 #{!} 4, H / 1920, Dummy2, H % 1920]>(copy)
                .tile::<m![Dummy2], 1, m![1 #{!} 4, H / 1920, Dummy2 = 1 #{!} 2, H % 1920]>(0),
        );
        x_lo.view().to_hbm_view(
            ...
            x2_hbm
                .view_mut()
                .tile::<m![Dummy256 / 64], 1, m![1 #{!} 4, H / 1920, Dummy2, H % 1920]>(copy)
                .tile::<m![Dummy2], 1, m![1 #{!} 4, H / 1920, Dummy2 = 1 #{!} 2, H % 1920]>(1),
        );
    }
...
fn stage_geglu_hi_lo_hbm(
...
) -> (HbmTensor<f8e4m3, Chip, m![Dummy256 / 64, L / 1920, Dummy2, L % 1920]>, HbmTensor<f32, Chip, m![L / 7680, 1 # 8]>) {
    ...
    // 4 real copies of every chunk, one store each.
    let mut x2_hbm: HbmTensor<f8e4m3, Chip, m![Dummy256 / 64, L / 1920, Dummy2, L % 1920]> = HbmTensor::new();
    for copy in 0..4 {
        x_hi.view().to_hbm_view(
            &mut ctx.tdma,
            x2_hbm
                .view_mut()
                .tile::<m![Dummy256 / 64], 1, m![1 #{!} 4, L / 1920, Dummy2, L % 1920]>(copy)
                .tile::<m![Dummy2], 1, m![1 #{!} 4, L / 1920, Dummy2 = 1 #{!} 2, L % 1920]>(0),
        );
        x_lo.view().to_hbm_view(
            &mut ctx.tdma,
            x2_hbm
                .view_mut()
                .tile::<m![Dummy256 / 64], 1, m![1 #{!} 4, L / 1920, Dummy2, L % 1920]>(copy)
                .tile::<m![Dummy2], 1, m![1 #{!} 4, L / 1920, Dummy2 = 1 #{!} 2, L % 1920]>(1),
        );
    }
...
pub(crate) fn feedforward(
    ctx: &mut Context,
    x2: &HbmTensor<f8e4m3, Chip, m![Dummy256 / 64, H / 1920, Dummy2, H % 1920]>,
    ...
    // half spread over the 4 copies.
    let x: DmTensor<f8e4m3, Chip, UpGateClusters, m![Dummy256 / 8, Dummy256 / 64, H / 1920], m![Dummy2, H % 1920]> =
    ...
    // the readers of a chunk spread over its 4 copies.
    let x: DmTensor<f8e4m3, Chip, DownClusters, m![Dummy256 / 32, Dummy256 / 64, L / 1920], m![Dummy2, L % 1920]> =
