...
/// V234: the same pass A with a 128-element fetch packet instead of 64.
///
/// Pass A costs ~5.1 static cycles per 64-element f4 packet whatever follows it -- 18,263 for
/// 3,600 packets fused with the contraction (V209), and 9,263 for 1,800 packets with the
/// contraction removed (V227's dequant). That is a per-packet cost, not a per-element one, so
/// halving the packet count should halve the pass.
///
/// 128 f4 is 64 B, exactly the Outer stage's packet cap, and the block partials come out as
/// `m![L % 30, H / 128, H / 16 % 8]` = `m![L % 30, H / 16]`, bit-for-bit the layout pass B already
/// reads, so nothing downstream changes. It also fills the Lane Folder's 8-element output bus:
/// Sequential with Packet::SIZE 4 carries four elements per cycle, with 8 it carries eight.
fn contract_up_gate_full_wide(
    ctx: &mut Context,
    x_trf: &TrfTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![1], m![Dummy2, H]>,
    packed: &DmTensor<f4e2m1, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H]>,
) -> DmTensor<f32, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H / 16]> {
    ctx.main
        .begin(packed.view())
        .fetch::<m![L % 30, H / 128, Dummy2], m![H % 128]>()
        .fetch_table_lookup::<f8e4m3>()
        .collect::<m![L % 30, H / 128, Dummy2, H / 32 % 4], m![H % 32]>()
        .contract_outer::<m![L % 30, H / 128, Dummy2], m![H % 128], _, _, _>(x_trf)
        .contract_packet::<m![H / 16 % 8]>()
        .contract_time::<m![L % 30, H / 128]>()
        .contract_lane::<m![L % 30, H / 128], m![H / 16 % 8]>(LaneMode::Sequential)
...
/// V234: `feedforward_v181` with a 128-element pass-A fetch packet.
pub(crate) fn feedforward_v234(
    ctx: &mut Context,
    x2: &HbmTensor<f8e4m3, Chip, m![Dummy2, H]>,
    erf_scale: &HbmTensor<f32, Chip, m![1 # 8]>,
    out_scale: &HbmTensor<f32, Chip, m![1 # 8]>,
    ...
    let up_partials = contract_up_gate_full_wide(ctx, &x_trf, &up_w);
    let gate_partials = contract_up_gate_full_wide(ctx, &x_trf, &gate_w);
    let mut up: DmTensor<f32, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, 1 # 8]> = DmTensor::new();
    let mut gate: DmTensor<f32, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, 1 # 8]> = DmTensor::new();
    reduce_up_gate_full_8(ctx, &up_partials, &up_scale, 0, &mut up);
    reduce_up_gate_full_8(ctx, &gate_partials, &gate_scale, 0, &mut gate);
