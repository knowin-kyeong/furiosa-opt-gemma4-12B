...
pub(crate) fn feedforward(
    ...
    let down0 = load_down_rows_32(ctx, down_weight_packed, 0);
    let down_scale: DmTensor<f8e4m3, Chip, DownClusters, DownRowsByColumns, m![H % 60, L / 16 % 120]> =
        down_weight_scale.to_dm(&mut ctx.tdma);
    let down1 = load_down_rows_16(ctx, down_weight_packed, 32);
    let down2 = load_down_rows_8(ctx, down_weight_packed, 48);
    ...
    let p = contract_down_rows_32(ctx, &x_trf, &down0);
    reduce_down_rows_32(ctx, &p, &down_scale, &inv_s_vrf, 0, &mut down);
...
down_tile_fns!(load_down_rows_32, contract_down_rows_32, reduce_down_rows_32, 32);
