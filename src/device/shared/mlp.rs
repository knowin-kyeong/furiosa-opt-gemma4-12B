...
pub(crate) fn feedforward_v181_r16(
    ctx: &mut Context,
    x2: &HbmTensor<f8e4m3, Chip, m![Dummy2, H]>,
    erf_scale: &HbmTensor<f32, Chip, m![1 # 8]>,
    out_scale: &HbmTensor<f32, Chip, m![1 # 8]>,
    ...
    let x = broadcast_x_r16::<UpGateClusters>(ctx, &x2);
    let x: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![Dummy2, H]> = unsafe { x.reshape() };
    let x_trf: TrfTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![1], m![Dummy2, H]> = ctx
        .sub
        .begin(x.view())
...
pub(crate) fn feedforward_v181_r8(
    ...
    let x = broadcast_x_r8::<UpGateClusters>(ctx, &x2);
    let x: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![Dummy2, H]> = unsafe { x.reshape() };
    let x_trf: TrfTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![1], m![Dummy2, H]> = ctx
        .sub
        .begin(x.view())
...
pub(crate) fn feedforward_v181_r4(
    ctx: &mut Context,
    x2: &HbmTensor<f8e4m3, Chip, m![Dummy2, H]>,
    erf_scale: &HbmTensor<f32, Chip, m![1 # 8]>,
    out_scale: &HbmTensor<f32, Chip, m![1 # 8]>,
    ...
    let x = broadcast_x_r4::<UpGateClusters>(ctx, &x2);
    let x: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![Dummy2, H]> = unsafe { x.reshape() };
    let x_trf: TrfTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![1], m![Dummy2, H]> = ctx
        .sub
        .begin(x.view())
