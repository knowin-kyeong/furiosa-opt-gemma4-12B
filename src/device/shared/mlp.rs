...
///
/// Written 8 times, one real `to_hbm_view` per copy, so that the replicated load spreads over
/// HBM channels instead of 64 slices reading the same bytes. V15..V41 got this for free by
...
pub(crate) fn stage_x_hi_lo_qkv_hbm(
    ctx: &mut Context,
    normalized: &DmTensor<f32, Chip, Cluster, ReducingSlices, m![H % 480]>,
) -> HbmTensor<f8e4m3, Chip, m![Dummy256 / 32, Dummy2, H]> {
    let x: DmTensor<bf16, Chip, Cluster, ReducingSlices, m![H % 480]> = ctx
        .main
        .begin(normalized.view())
        .fetch::<m![H / 8 % 60], m![H % 8]>()
    ...
    let (x_hi, x_lo) = hi_lo_reducing(ctx, &x, &s_vrf);
    let mut x2_hbm: HbmTensor<f8e4m3, Chip, m![Dummy256 / 32, Dummy2, H]> = HbmTensor::new();
    for copy in 0..8 {
        x_hi.view().to_hbm_view(
            &mut ctx.tdma,
            x2_hbm
                .view_mut()
                .tile::<m![Dummy256 / 32], 1, m![1 #{!} 8, Dummy2, H]>(copy)
                .tile::<m![Dummy2], 1, m![1 #{!} 8, Dummy2 = 1 #{!} 2, H]>(0),
        );
        x_lo.view().to_hbm_view(
            &mut ctx.tdma,
            x2_hbm
                .view_mut()
                .tile::<m![Dummy256 / 32], 1, m![1 #{!} 8, Dummy2, H]>(copy)
                .tile::<m![Dummy2], 1, m![1 #{!} 8, Dummy2 = 1 #{!} 2, H]>(1),
        );
    }
    x2_hbm
}
