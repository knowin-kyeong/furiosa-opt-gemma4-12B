...
fn stage_geglu_hi_lo_hbm_g1(
    ...
    let (x_hi, x_lo) = hi_lo_gathered(ctx, x, &s_vrf);
    // g1: both geglu pieces go out in one DMA command. They already sit on the same sixteen
    // slices, so copying them into one [Dummy2, L % 480] buffer costs two cheap on-chip passes
    // and removes the more expensive of the kernel's two store pairs (1,653 static cycles each).
    let mut x2: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsGathered, m![Dummy2, L % 480]> =
        DmTensor::new();
    ctx.main
        .begin(x_hi.view())
...
pub(crate) fn feedforward_g1(
    ...
    let g = geglu_full(ctx, up, gate, erf_scale, out_scale);
    let x = gather_pack_full(ctx, &g);
    let (x2_hbm, inv_s_hbm) = stage_geglu_hi_lo_hbm_g1(ctx, &x);
    let inv_s_vrf = broadcast_inv_s_down(ctx, &inv_s_hbm);
