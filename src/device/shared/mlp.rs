...
pub(crate) fn feedforward(
    ...
    let p = contract_down_rows_28(ctx, &x_trf, &down0);
    reduce_down_rows_28(ctx, &p, &down_scale, &inv_s_vrf, 0, &mut down);
    let p = contract_down_rows_28(ctx, &x_trf, &down1);
    reduce_down_rows_28(ctx, &p, &down_scale, &inv_s_vrf, 28, &mut down);
...
down_tile_fns!(load_down_rows_28, contract_down_rows_28, reduce_down_rows_28, 28);
