...
// V202: a pass helper that reads an already loaded buffer, so the load height and the pass height
// are independent. `$rows` is the pass height and `$bulk` the height of the buffer the view is cut
// from, which a tile of a taller buffer carries in its type.
macro_rules! down_view_fns {
    ($contract:ident, $rows:literal, $bulk:literal) => {
        fn $contract(
            ctx: &mut Context,
            x_trf: &TrfTensor<f8e4m3, Chip, DownClusters, DownRowsByColumns, m![1], m![Dummy2, L % 1920]>,
            packed: DmTensorView<'_, f4e2m1, Chip, DownClusters, DownRowsByColumns, m![H % 60 = $rows # $bulk, L % 1920]>,
        ) -> DmTensor<f32, Chip, DownClusters, DownRowsByColumns, m![H % 60 = $rows, L / 16 % 120]> {
            ctx.main
                .begin(packed)
                .fetch::<m![H % 60 = $rows, L / 64 % 30, Dummy2], m![L % 64]>()
...
/// Loads `$rows` rows of the down weight in one command, starting at row `offset`.
macro_rules! down_bulk_fns {
    ($load:ident, $rows:literal) => {
        fn $load(
            ctx: &mut Context,
            packed: &HbmTensor<f4e2m1, Chip, m![H, L]>,
            offset: usize,
...
down_view_fns!(contract_down_v4_12, 4, 12);
down_view_fns!(contract_down_v4_28, 4, 28);
down_view_fns!(contract_down_v4_60, 4, 60);
down_view_fns!(contract_down_v8_12, 8, 12);
down_view_fns!(contract_down_v8_28, 8, 28);
down_view_fns!(contract_down_v8_60, 8, 60);
down_view_fns!(contract_down_v16_16, 16, 16);
down_view_fns!(contract_down_v16_28, 16, 28);
down_view_fns!(contract_down_v16_32, 16, 32);
down_view_fns!(contract_down_v16_60, 16, 60);
down_bulk_fns!(load_down_bulk_12, 12);
down_bulk_fns!(load_down_bulk_16, 16);
down_bulk_fns!(load_down_bulk_28, 28);
down_bulk_fns!(load_down_bulk_32, 32);
down_bulk_fns!(load_down_bulk_60, 60);

pub(crate) fn feedforward_d1(
    ctx: &mut Context,
    x2: &HbmTensor<f8e4m3, Chip, m![Dummy2, H]>,
    erf_scale: &HbmTensor<f32, Chip, m![1 # 8]>,
    out_scale: &HbmTensor<f32, Chip, m![1 # 8]>,
    ...
    let gate_w: DmTensor<f4e2m1, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H]> = gate_weight_packed.to_dm(&mut ctx.tdma);
    let gate_scale: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H / 16]> = gate_weight_scale.to_dm(&mut ctx.tdma);
    // V202: the down weight arrives in 1 command(s) instead of five.
    let bulk0 = load_down_bulk_60(ctx, down_weight_packed, 0);
    let down_scale: DmTensor<f8e4m3, Chip, DownClusters, DownRowsByColumns, m![H % 60, L / 16 % 120]> =
        down_weight_scale.to_dm(&mut ctx.tdma);
    ...
    let mut down: DmTensor<bf16, Chip, DownClusters, DownRows, m![H % 60]> = DmTensor::new();
    let p = contract_down_v16_60(ctx, &x_trf, bulk0.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 60, L % 1920]>(0));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 0, &mut down);
    let p = contract_down_v16_60(ctx, &x_trf, bulk0.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 60, L % 1920]>(16));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 16, &mut down);
    let p = contract_down_v16_60(ctx, &x_trf, bulk0.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 60, L % 1920]>(32));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 32, &mut down);
    let p = contract_down_v8_60(ctx, &x_trf, bulk0.view().tile::<m![H % 60], 8, m![H % 60 = 8 # 60, L % 1920]>(48));
    reduce_down_rows_8(ctx, &p, &down_scale, &inv_s_vrf, 48, &mut down);
    let p = contract_down_v4_60(ctx, &x_trf, bulk0.view().tile::<m![H % 60], 4, m![H % 60 = 4 # 60, L % 1920]>(56));
    reduce_down_rows_4(ctx, &p, &down_scale, &inv_s_vrf, 56, &mut down);
...
pub(crate) fn feedforward_d2(
    ...
    let gate_w: DmTensor<f4e2m1, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H]> = gate_weight_packed.to_dm(&mut ctx.tdma);
    let gate_scale: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H / 16]> = gate_weight_scale.to_dm(&mut ctx.tdma);
    // V202: the down weight arrives in 2 command(s) instead of five.
    let bulk0 = load_down_bulk_32(ctx, down_weight_packed, 0);
    let bulk1 = load_down_bulk_28(ctx, down_weight_packed, 32);
    let down_scale: DmTensor<f8e4m3, Chip, DownClusters, DownRowsByColumns, m![H % 60, L / 16 % 120]> =
        down_weight_scale.to_dm(&mut ctx.tdma);
    ...
    let mut down: DmTensor<bf16, Chip, DownClusters, DownRows, m![H % 60]> = DmTensor::new();
    let p = contract_down_v16_32(ctx, &x_trf, bulk0.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 32, L % 1920]>(0));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 0, &mut down);
    let p = contract_down_v16_32(ctx, &x_trf, bulk0.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 32, L % 1920]>(16));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 16, &mut down);
    let p = contract_down_v16_28(ctx, &x_trf, bulk1.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 28, L % 1920]>(0));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 32, &mut down);
    let p = contract_down_v8_28(ctx, &x_trf, bulk1.view().tile::<m![H % 60], 8, m![H % 60 = 8 # 28, L % 1920]>(16));
    reduce_down_rows_8(ctx, &p, &down_scale, &inv_s_vrf, 48, &mut down);
    let p = contract_down_v4_28(ctx, &x_trf, bulk1.view().tile::<m![H % 60], 4, m![H % 60 = 4 # 28, L % 1920]>(24));
    reduce_down_rows_4(ctx, &p, &down_scale, &inv_s_vrf, 56, &mut down);
...
pub(crate) fn feedforward_d3(
    ...
    let gate_w: DmTensor<f4e2m1, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H]> = gate_weight_packed.to_dm(&mut ctx.tdma);
    let gate_scale: DmTensor<f8e4m3, Chip, UpGateClusters, UpGateRowsFull, m![L % 30, H / 16]> = gate_weight_scale.to_dm(&mut ctx.tdma);
    // V202: the down weight arrives in 3 command(s) instead of five.
    let bulk0 = load_down_bulk_16(ctx, down_weight_packed, 0);
    let bulk1 = load_down_bulk_32(ctx, down_weight_packed, 16);
    let bulk2 = load_down_bulk_12(ctx, down_weight_packed, 48);
    let down_scale: DmTensor<f8e4m3, Chip, DownClusters, DownRowsByColumns, m![H % 60, L / 16 % 120]> =
        down_weight_scale.to_dm(&mut ctx.tdma);
    ...
    let mut down: DmTensor<bf16, Chip, DownClusters, DownRows, m![H % 60]> = DmTensor::new();
    let p = contract_down_v16_16(ctx, &x_trf, bulk0.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 16, L % 1920]>(0));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 0, &mut down);
    let p = contract_down_v16_32(ctx, &x_trf, bulk1.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 32, L % 1920]>(0));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 16, &mut down);
    let p = contract_down_v16_32(ctx, &x_trf, bulk1.view().tile::<m![H % 60], 16, m![H % 60 = 16 # 32, L % 1920]>(16));
    reduce_down_rows_16(ctx, &p, &down_scale, &inv_s_vrf, 32, &mut down);
    let p = contract_down_v8_12(ctx, &x_trf, bulk2.view().tile::<m![H % 60], 8, m![H % 60 = 8 # 12, L % 1920]>(0));
    reduce_down_rows_8(ctx, &p, &down_scale, &inv_s_vrf, 48, &mut down);
    let p = contract_down_v4_12(ctx, &x_trf, bulk2.view().tile::<m![H % 60], 4, m![H % 60 = 4 # 12, L % 1920]>(8));
    reduce_down_rows_4(ctx, &p, &down_scale, &inv_s_vrf, 56, &mut down);
