...
pub(crate) fn stage_x_hi_lo_hbm(
    ...
    // Eight copies (the `Dummy8` axis, absent from the source, replicates the store): each
    // column half is read by 256 slices, and descriptors that all read the same HBM bytes run
    // well below the DMA rate (V15, V36), so the load spreads over the copies.
    ...
    x_hi.view().to_hbm_view(
        &mut ctx.tdma,
        x2_hbm.view_mut().tile::<m![Dummy2], 1, m![Dummy8, H / 1920, Dummy2 = 1 #{!} 2, H % 1920]>(0),
    );
    x_lo.view().to_hbm_view(
        &mut ctx.tdma,
        x2_hbm.view_mut().tile::<m![Dummy2], 1, m![Dummy8, H / 1920, Dummy2 = 1 #{!} 2, H % 1920]>(1),
    );
...
fn stage_geglu_hi_lo_hbm(
    ...
    // Eight copies of every chunk (see stage_x_hi_lo_hbm): each chunk is read by 64 slices.
    let (x_hi, x_lo) = hi_lo_gathered(ctx, x, &s_vrf);
    let mut x2_hbm: HbmTensor<f8e4m3, Chip, m![Dummy8, L / 1920, Dummy2, L % 1920]> = HbmTensor::new();
    x_hi.view().to_hbm_view(
        &mut ctx.tdma,
        x2_hbm.view_mut().tile::<m![Dummy2], 1, m![Dummy8, L / 1920, Dummy2 = 1 #{!} 2, L % 1920]>(0),
    );
    x_lo.view().to_hbm_view(
        &mut ctx.tdma,
        x2_hbm.view_mut().tile::<m![Dummy2], 1, m![Dummy8, L / 1920, Dummy2 = 1 #{!} 2, L % 1920]>(1),
    );
...
pub(crate) fn feedforward(
    ...
    // Each slice needs only its 1920-wide half of x (both f8 pieces, one DMA). The slices are
    // spread over the eight HBM copies (a copy axis in the slice mapping selects a copy, an
    // absent axis replicates); the tensor is then relabelled with the projection's row groups.
    ...
    // the 64 readers of a chunk spread over its eight copies.
