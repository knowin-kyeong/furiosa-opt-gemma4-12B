...
/// V307 harness kernel: V301_submit qkv with RoPE rows gathered per cluster.
#[device(chip = 1)]
pub fn sliding_project_qkv_g1(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    let (q, k) = sliding::rope::apply_rope_heads_g1::<layout::HeadClusters, layout::HeadSlicesPerCluster>(
        ctx,
        &q,
        &k,
        rope_offset,
