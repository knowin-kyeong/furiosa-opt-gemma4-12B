...
pub fn sliding_project_qkv_lt(
    ...
    // V383: the RoPE rows are staged by one HBM store and one ExplicitSync (`apply_rope_heads_cc_1s`).
    // V387: the RoPE staging store, sync and reload are scheduled after the K projection (`apply_rope_heads_cc_lt`).
    let (q, k) = sliding::rope::apply_rope_heads_cc_lt::<layout::HeadClusters, layout::HeadSlicesPerCluster>(
        ctx,
        &q,
        &k,
        rope_offset,
