...
#[device(chip = 1)]
pub fn sliding_attention_output_a1(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    // each slice's Qs chunk straight from there instead of broadcasting x through the switch.
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let x_hbm = sliding::projection::project_output_a1(ctx, x, o_weight);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
#[device(chip = 1)]
pub fn sliding_attention_output_a2(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    // each slice's Qs chunk straight from there instead of broadcasting x through the switch.
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let x_hbm = sliding::projection::project_output_a2(ctx, x, o_weight);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
pub fn decoder_feedforward_f1(
    ...
    // The up/gate stage runs on whole rows (V181): each slice's f4 rows and block scales are one
    // contiguous HBM segment each; a segmented load costs twice per byte on hardware (V174).
    let x = shared::mlp::feedforward_f1(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
...
pub fn sliding_project_qkv_q1(
    ...
    let q_weight = sliding::projection::load_query_weight(ctx, q_weight);
    // q1 probe: a second RoPE cos/sin round trip (gather -> HBM -> load), the shape the real
    // one uses, so the increment prices the 1 KB table's trip through HBM.
    let cos_row2: DmTensor<bf16, Chip, Cluster, Slice, m![Ds]> = cos.dma_gather_scaled(rope_offset);
    let sin_row2: DmTensor<bf16, Chip, Cluster, Slice, m![Ds]> = sin.dma_gather_scaled(rope_offset);
    let mut cos_hbm2: HbmTensor<bf16, Chip, m![Ds]> = HbmTensor::new();
    cos_row2.view().to_hbm_view(&mut ctx.tdma, cos_hbm2.view_mut());
    let mut sin_hbm2: HbmTensor<bf16, Chip, m![Ds]> = HbmTensor::new();
    sin_row2.view().to_hbm_view(&mut ctx.tdma, sin_hbm2.view_mut());
    let cos2: DmTensor<bf16, Chip, Cluster, Slice, m![Ds]> = cos_hbm2.to_dm(&mut ctx.tdma);
    let sin2: DmTensor<bf16, Chip, Cluster, Slice, m![Ds]> = sin_hbm2.to_dm(&mut ctx.tdma);
    let _cos2_vrf: VrfTensor<f32, Chip, Cluster, Slice, m![Ds]> = ctx
        .sub
        .begin(cos2.view())
        ...
        .to_vrf();
    let _sin2_vrf: VrfTensor<f32, Chip, Cluster, Slice, m![Ds]> = ctx
        .sub
        .begin(sin2.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
