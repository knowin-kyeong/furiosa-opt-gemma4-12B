...
/// V385 arm od: production qkv with the RoPE rows computed on chip.
#[device(chip = 1)]
pub fn sliding_project_qkv_od(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // V383: the RoPE rows are staged by one HBM store and one ExplicitSync (`apply_rope_heads_cc_1s`).
    // V385: the RoPE rows are computed on the head slices; the cos/sin tables are not read.
    let (q, k) = sliding::rope::apply_rope_heads_cc_od::<layout::HeadClusters, layout::HeadSlicesPerCluster>(
        ctx,
        &q,
        &k,
        rope_offset,
...
/// V385 arm oe: production qkv with the RoPE rows computed on chip.
#[device(chip = 1)]
pub fn sliding_project_qkv_oe(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // V383: the RoPE rows are staged by one HBM store and one ExplicitSync (`apply_rope_heads_cc_1s`).
    // V385: the RoPE rows are computed on the head slices; the cos/sin tables are not read.
    let (q, k) = sliding::rope::apply_rope_heads_cc_oe::<layout::HeadClusters, layout::HeadSlicesPerCluster>(
        ctx,
        &q,
        &k,
        rope_offset,
