...
/// V318 harness kernel: V313_submit attn_out with the clusters alternating every 120-row group.
#[device(chip = 1)]
pub fn sliding_attention_output_cx(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    // each slice's Qs chunk straight from there instead of broadcasting x through the switch.
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let x_hbm = sliding::projection::project_output_cx(ctx, x, o_weight);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing_aligned::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
/// V340 harness kernel (arm ut): uneven tiles -- rows 0..96 of every 120-row group on both clusters, rows 96..120 of all
/// groups on cluster 0 alone (cluster 1's slices carry fewer bytes, so it stops lagging); the tail rows reach the norm
/// by a DM-to-DM move on cluster 0 instead of the HBM round trip.
#[device(chip = 1)]
pub fn sliding_attention_output_ut(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
...
/// V342 harness kernel (arm ua): three uneven tiles -- rows 0..72 and 72..96 on both clusters (cluster 1 keeps the
/// production two-tile rhythm), rows 96..120 of all groups on cluster 0 alone; tail rows overwritten after the reload.
#[device(chip = 1)]
pub fn sliding_attention_output_ua(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let (stored, tails) = sliding::uneven3::project_output_u3(ctx, x, o_weight);
    let x = sliding::uneven3::load_reducing_tails_after::<Cluster>(ctx, &stored, &tails);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    let residual = shared::rmsnorm::normalize_add_scaled_reduced::<Cluster>(ctx, &x, o_weight_scale, post_attn_rms_weight, &residual);
    residual.view().to_hbm_view(&mut ctx.tdma, residual_hbm.view_mut());
}

/// V342 harness kernel (arm ub): as ua, but the tail rows are written into the reload buffer first and the reload after
/// the store's sync carries only rows 0..96 of every group.
#[device(chip = 1)]
pub fn sliding_attention_output_ub(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let (stored, tails) = sliding::uneven3::project_output_u3(ctx, x, o_weight);
    let x = sliding::uneven3::load_reducing_tails_first::<Cluster>(ctx, &stored, &tails);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    let residual = shared::rmsnorm::normalize_add_scaled_reduced::<Cluster>(ctx, &x, o_weight_scale, post_attn_rms_weight, &residual);
    residual.view().to_hbm_view(&mut ctx.tdma, residual_hbm.view_mut());
}
