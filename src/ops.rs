...
#[device(chip = 1)]
pub fn sliding_project_qkv_v155a(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
...
#[device(chip = 1)]
pub fn sliding_project_qkv_v155g(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    let x: DmTensor<f8e4m3, Chip, layout::BothClusters, m![Dummy8, Dummy256 / 8], m![Dummy2, H]> = ctx
        ...
        .begin(x8.view())
        .fetch::<m![Dummy2], m![H]>()
        .switch::<m![Dummy8, Dummy256 / 8], m![Dummy2]>(SwitchConfig::CustomBroadcast { ring_size: 32 })
        .collect::<m![Dummy2, H / 32], m![H % 32]>()
        .commit_trim::<m![H % 32]>()
        .commit();
    let x: DmTensor<f8e4m3, Chip, layout::BothClusters, Replicated, m![Dummy2, H]> = unsafe { x.reshape() };
...
#[device(chip = 1)]
pub fn sliding_project_qkv_v155e(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // slices (32 x 240 flits = 7,680 static), the documented CustomBroadcast form: a `1 # 32`
    // padding slot turned into a live tile axis.
    let x16: DmTensor<f8e4m3, Chip, layout::BothClusters, m![Gf, 1 # 16], m![Dummy2, H]> = x2_hbm.to_dm(&mut ctx.tdma);
    let x: DmTensor<f8e4m3, Chip, layout::BothClusters, m![Gf, Dummy256 / 16], m![Dummy2, H]> = ctx
        .main
        .begin(x16.view())
        .fetch::<m![Dummy2], m![H]>()
...
#[device(chip = 1)]
pub fn sliding_project_qkv_v155d(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // slices (32 x 240 flits = 7,680 static), the documented CustomBroadcast form: a `1 # 32`
    // padding slot turned into a live tile axis.
    let x4: DmTensor<f8e4m3, Chip, layout::BothClusters, m![Ns / 2, 1 # 64], m![Dummy2, H]> = x2_hbm.to_dm(&mut ctx.tdma);
    let x: DmTensor<f8e4m3, Chip, layout::BothClusters, m![Ns / 2, Dummy256 / 4], m![Dummy2, H]> = ctx
        .main
        .begin(x4.view())
        .fetch::<m![Dummy2], m![H]>()
        .switch::<m![Ns / 2, Dummy256 / 4], m![Dummy2]>(SwitchConfig::CustomBroadcast { ring_size: 64 })
        .collect::<m![Dummy2, H / 32], m![H % 32]>()
        .commit_trim::<m![H % 32]>()
        .commit();
    let x: DmTensor<f8e4m3, Chip, layout::BothClusters, Replicated, m![Dummy2, H]> = unsafe { x.reshape() };
...
#[device(chip = 1)]
pub fn sliding_attention_output_v156(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    sliding::projection::project_output_into_v156(ctx, x, o_weight, &mut x_hbm);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
