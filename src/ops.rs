...
/// V292 harness kernel: qkv with x replicated on chip.
#[device(chip = 1)]
pub fn sliding_project_qkv_sw(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // V292: x staged on both clusters and replicated on chip (no HBM hop, so no ExplicitSync).
...
/// V299 harness kernel: V292 plus Q/K/V weights interleaved within each head.
#[device(chip = 1)]
pub fn sliding_project_qkv_hi(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
