...
/// V316 harness kernel: V313_submit qkv with the rotate/sin fusion.
#[device(chip = 1)]
pub fn sliding_project_qkv_rf(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    let (q, k) = sliding::rope::apply_rope_heads_rf::<layout::HeadClusters, layout::HeadSlicesPerCluster>(
        ctx,
        &q,
        &k,
        rope_offset,
