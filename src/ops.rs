...
/// V288 harness kernel (rs).
#[device(chip = 1)]
pub fn sliding_project_qkv_rs(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    let (q, k) = sliding::rope::apply_rope_heads_rs::<layout::HeadClusters, layout::HeadSlicesPerCluster>(
        ctx,
        &q,
        &k,
        rope_offset,
...
pub fn sliding_project_qkv_h2(
    ...
    let q_weight = sliding::projection::load_query_weight_h2(ctx, q_weight);
    ...
    let x: DmTensor<f8e4m3, Chip, m![Qs / 2048], m![Dummy8, Dummy256 / 8], m![Dummy2, H]> = ctx
        ...
        .commit();
    let x: DmTensor<f8e4m3, Chip, layout::BothClusters, Replicated, m![Dummy2, H]> = unsafe { x.reshape() };
    let k_weight = sliding::projection::load_kv_weight_h2(ctx, k_weight);
    let v_weight = sliding::projection::load_kv_weight_h2(ctx, v_weight);

    // q, k and v come back one head per slice; the head-wise RMSNorms and RoPE stay in that
    // layout (no transposes in or broadcasts out) and the outputs are written from it.
    let q = sliding::projection::project_query_h2(ctx, &x, &q_weight);
    let (k, v) = sliding::projection::project_key_value_h2(ctx, &x, &k_weight, &v_weight);
...
/// V288 harness kernel (hr).
#[device(chip = 1)]
pub fn sliding_project_qkv_hr(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
