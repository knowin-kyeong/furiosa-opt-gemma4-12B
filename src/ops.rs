...
pub fn decoder_feedforward_ia(
    ...
    // V349: the geglu hi/lo pieces are staged by one store instead of two (13/16 paired jobs, -0.43%).
    // V366: the tail multiply (down_global_scale x out_scale) is folded into the post-FF norm; g is made once early.
    // V384: the geglu 1/s is stored as one aligned 256 B block per cluster (no partial-line write under the sync).
    let (x, g) = shared::mlp::feedforward_fo_t1_ia(
        ctx,
        x_rep,
        erf_all,
        out_tail,
