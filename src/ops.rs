...
#[device(chip = 1)]
pub fn sliding_project_qkv_v160(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    let q_weight = sliding::projection::load_query_weight_16(ctx, q_weight);
    ...
    let x: DmTensor<f8e4m3, Chip, m![Qs / 2048], m![Dummy8, Dummy256 / 8], m![Dummy2, H]> = ctx
        ...
        .commit();
    let x: DmTensor<f8e4m3, Chip, layout::BothClusters, Replicated, m![Dummy2, H]> = unsafe { x.reshape() };
    let k_weight = sliding::projection::load_kv_weight_8(ctx, k_weight);
    let v_weight = sliding::projection::load_kv_weight_8(ctx, v_weight);

    // q, k and v come back one head per slice; the head-wise RMSNorms and RoPE stay in that
    // layout (no transposes in or broadcasts out) and the outputs are written from it.
    let q = sliding::projection::project_query_16(ctx, &x, &q_weight);
    let (k, v) = sliding::projection::project_key_value_8(ctx, &x, &k_weight, &v_weight);
...
#[device(chip = 1)]
pub fn sliding_project_qkv_v164(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // V164: q goes out through the indirect-DMA path (one scatter at a zero index made on-device
    // as kv_offset - kv_offset) instead of a to_hbm_view from the padded head layout, which V146
    // showed costs ~4k more than a scatter of the same bytes.
    let off: DmTensor<i32, Chip, Cluster, Slice, m![1 # 8]> = kv_offset.to_dm(&mut ctx.tdma);
    let zero: DmTensor<i32, Chip, Cluster, Slice, m![1 # 8]> = ctx
        .sub
        .begin(off.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        ...
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_stash()
        .vector_fxp(FxpBinaryOp::SubFxp, Stash)
        .vector_final()
        .commit_trim::<m![1 # 8]>()
        .commit();
    let mut zero_hbm: HbmTensor<i32, Chip, m![1 # 8]> = HbmTensor::new();
