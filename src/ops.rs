...
/// V215 probe p1: one extra 15.73 MB query-weight load as 1 command x 8 rows, 30,720-byte runs.
#[device(chip = 1)]
pub fn sliding_project_qkv_p1(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    sliding::projection::probe_q_rows_8x1(ctx, q_weight);
    let q_weight = sliding::projection::load_query_weight(ctx, q_weight);
...
/// V215 probe p2: one extra 15.73 MB query-weight load as 2 commands x 4 rows, 15,360-byte runs.
#[device(chip = 1)]
pub fn sliding_project_qkv_p2(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    sliding::projection::probe_q_rows_4x2(ctx, q_weight);
    let q_weight = sliding::projection::load_query_weight(ctx, q_weight);
...
/// V215 probe p4: one extra 15.73 MB query-weight load as 4 commands x 2 rows, 7,680-byte runs.
#[device(chip = 1)]
pub fn sliding_project_qkv_p4(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    sliding::projection::probe_q_rows_2x4(ctx, q_weight);
    let q_weight = sliding::projection::load_query_weight(ctx, q_weight);
...
/// V215 probe p8: one extra 15.73 MB query-weight load as 8 commands x 1 row, 3,840-byte runs.
#[device(chip = 1)]
pub fn sliding_project_qkv_p8(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    sliding::projection::probe_q_rows_1x8(ctx, q_weight);
    let q_weight = sliding::projection::load_query_weight(ctx, q_weight);
