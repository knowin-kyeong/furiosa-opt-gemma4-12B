...
pub fn sliding_attention_output_bc(
    ...
    let x_hbm = sliding::projection::project_output(ctx, x, o_weight);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    // BC: the norm tail runs on both clusters, so neither PE group sits idle at the post-store sync.
    let x = shared::rmsnorm::load_reducing_aligned::<m![Qs / 2048]>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<m![Qs / 2048]>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
    let residual = shared::rmsnorm::normalize_add_scaled_reduced::<m![Qs / 2048]>(ctx, &x, o_weight_scale, post_attn_rms_weight, &residual);
    residual.view().to_hbm_view(&mut ctx.tdma, residual_hbm.view_mut());
}
