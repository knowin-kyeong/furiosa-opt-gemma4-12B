...
/// V362 arm ug: production with the RoPE rows gathered on both clusters by an on-chip index.
#[device(chip = 1)]
pub fn sliding_project_qkv_ug(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    let (q, k) = sliding::rope::apply_rope_heads_cc_ug::<layout::HeadClusters, layout::HeadSlicesPerCluster>(
        ctx,
        &q,
        &k,
        rope_offset,
...
/// V363 arm ix: production with the RoPE rows gathered on both clusters through one explicit HBM index.
#[device(chip = 1)]
pub fn sliding_project_qkv_ix(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    let (q, k) = sliding::rope::apply_rope_heads_cc_ix::<layout::HeadClusters, layout::HeadSlicesPerCluster>(
        ctx,
        &q,
        &k,
        rope_offset,
