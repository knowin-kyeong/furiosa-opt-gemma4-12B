...
/// E1/V262: Q/K/V weight packets fetched once and replayed over the two f8 pieces.
#[device(chip = 1)]
pub fn sliding_project_qkv_bc(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // q, k and v come back one head per slice; the head-wise RMSNorms and RoPE stay in that
    // layout (no transposes in or broadcasts out) and the outputs are written from it.
    let q = sliding::projection::project_query_bc(ctx, &x, &q_weight);
    let (k, v) = sliding::projection::project_key_value_bc(ctx, &x, &k_weight, &v_weight);
