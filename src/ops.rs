...
#[device(chip = 1)]
pub fn sliding_attention_output_v161(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    sliding::projection::project_output_into_v161(ctx, x, o_weight, &mut x_hbm);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
#[device(chip = 1)]
pub fn decoder_feedforward_v159(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    let x = shared::mlp::feedforward_v159(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
...
#[device(chip = 1)]
pub fn sliding_project_qkv_v157a(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
...
#[device(chip = 1)]
pub fn sliding_project_qkv_v157b(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // padding slot turned into a live tile axis.
    let x8: DmTensor<f8e4m3, Chip, layout::BothClusters, m![Dummy8, 1 # 32], m![Dummy2, H]> = x2_hbm.to_dm(&mut ctx.tdma);
    let x8: DmTensor<f8e4m3, Chip, m![Qs / 2048], m![Dummy8, 1 # 32], m![Dummy2, H]> = unsafe { x8.reshape() };
    let x: DmTensor<f8e4m3, Chip, m![Qs / 2048], m![Dummy8, Dummy256 / 8], m![Dummy2, H]> = ctx
        .main
        .begin(x8.view())
        .fetch::<m![Dummy2, H / 32], m![H % 32]>()
...
#[device(chip = 1)]
pub fn sliding_project_qkv_v157e(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // slices (32 x 240 flits = 7,680 static), the documented CustomBroadcast form: a `1 # 32`
    // padding slot turned into a live tile axis.
    let x16: DmTensor<f8e4m3, Chip, m![Qs / 2048], m![Gf, 1 # 16], m![Dummy2, H]> = x2_hbm.to_dm(&mut ctx.tdma);
    let x: DmTensor<f8e4m3, Chip, m![Qs / 2048], m![Gf, Dummy256 / 16], m![Dummy2, H]> = ctx
        .main
        .begin(x16.view())
        .fetch::<m![Dummy2], m![H]>()
        .switch::<m![Gf, Dummy256 / 16], m![Dummy2]>(SwitchConfig::CustomBroadcast { ring_size: 16 })
        .collect::<m![Dummy2, H / 32], m![H % 32]>()
        .commit_trim::<m![H % 32]>()
        .commit();
    let x: DmTensor<f8e4m3, Chip, layout::BothClusters, Replicated, m![Dummy2, H]> = unsafe { x.reshape() };
...
#[device(chip = 1)]
pub fn sliding_attention_output_v162(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let mut x_hbm: HbmTensor<bf16, Chip, m![H]> = HbmTensor::new();
    sliding::projection::project_output_into_v162(ctx, x, o_weight, &mut x_hbm);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
#[device(chip = 1)]
pub fn sliding_attention_output_v163(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let mut x_hbm: HbmTensor<bf16, Chip, m![H]> = HbmTensor::new();
    sliding::projection::project_output_into_v163(ctx, x, o_weight, &mut x_hbm);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
#[device(chip = 1)]
pub fn sliding_attention_output_v167(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let mut x_hbm: HbmTensor<bf16, Chip, m![H]> = HbmTensor::new();
    sliding::projection::project_output_into_v167(ctx, x, o_weight, &mut x_hbm);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
#[device(chip = 1)]
pub fn decoder_feedforward_v168(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    // loaded into its reducing layout above, overwritten by the final store) instead of a fresh
    // HBM scratch.
    let x = shared::mlp::feedforward_v168(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
...
pub fn decoder_feedforward_f3(
    ...
    // loaded into its reducing layout above, overwritten by the final store) instead of a fresh
    // HBM scratch.
    let x = shared::mlp::feedforward_f3(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
...
pub fn decoder_feedforward_f7(
    ...
    // loaded into its reducing layout above, overwritten by the final store) instead of a fresh
    // HBM scratch.
    let x = shared::mlp::feedforward_f7(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
...
pub fn decoder_feedforward_f6(
    ...
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
    // f6: no post-FF RMSNorm / residual / gate (ablation) - the down output is the result.
    let _ = residual;
    x.view().to_hbm_view(&mut ctx.tdma, residual_hbm.view_mut());
}

#[device(chip = 1)]
pub fn sliding_project_qkv_v172(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // layout (no transposes in or broadcasts out) and the outputs are written from it.
    let q = sliding::projection::project_query(ctx, &x, &q_weight);
    // v172 probe: the Q contraction a second time, stored to scratch, so that the excess over
    // the pass's own time reads the DMA/fetch interference during the K/V weight streams.
    let q2 = sliding::projection::project_query(ctx, &x, &q_weight);
    let mut q2_hbm: HbmTensor<bf16, Chip, m![Ns, Gs, Ds]> = HbmTensor::new();
    q2.view().to_hbm_view(&mut ctx.tdma, q2_hbm.view_mut());
    let (k, v) = sliding::projection::project_key_value(ctx, &x, &k_weight, &v_weight);
...
#[device(chip = 1)]
pub fn sliding_attention_output_v173(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let mut x_hbm: HbmTensor<bf16, Chip, m![H]> = HbmTensor::new();
    sliding::projection::project_output_into_v173(ctx, x, o_weight, &mut x_hbm);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
#[device(chip = 1)]
pub fn decoder_feedforward_f174a(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    let (x2_hbm, erf_scale, out_scale) =
        shared::mlp::stage_x_hi_lo_hbm(ctx, &x, up_global_scale, gate_global_scale);
    let mut down_hbm: HbmTensor<bf16, Chip, m![H]> = HbmTensor::new();
    let x = shared::mlp::feedforward_f174a(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
...
#[device(chip = 1)]
pub fn decoder_feedforward_f174b(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    let (x2_hbm, erf_scale, out_scale) =
        shared::mlp::stage_x_hi_lo_hbm(ctx, &x, up_global_scale, gate_global_scale);
    let mut down_hbm: HbmTensor<bf16, Chip, m![H]> = HbmTensor::new();
    let x = shared::mlp::feedforward_f174b(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
...
#[device(chip = 1)]
pub fn decoder_feedforward_v181(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
...
#[device(chip = 1)]
pub fn decoder_feedforward_v183(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    let (x2_hbm, erf_scale, out_scale) =
        shared::mlp::stage_x_hi_lo_hbm_full(ctx, &x, up_global_scale, gate_global_scale);
    let mut down_hbm: HbmTensor<bf16, Chip, m![H]> = HbmTensor::new();
    let x = shared::mlp::feedforward_v183(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
...
#[device(chip = 1)]
pub fn decoder_feedforward_v184(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    let (x2_hbm, erf_scale, out_scale) =
        shared::mlp::stage_x_hi_lo_hbm_full(ctx, &x, up_global_scale, gate_global_scale);
    let mut down_hbm: HbmTensor<bf16, Chip, m![H]> = HbmTensor::new();
    let x = shared::mlp::feedforward_v184(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
