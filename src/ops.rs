...
pub fn sliding_project_qkv_v239(
    ...
    let mut qk: DmTensor<bf16, Chip, layout::HeadClusters, layout::HeadSlicesPerCluster, m![Ns / 2, Ds]> =
        DmTensor::new();
    sliding::rmsnorm::head_normalize_query(ctx, &q, &q_scale_vrf, &q_weight_vrf, &q_rms_vrf, &mut qk);
    sliding::rmsnorm::head_normalize_row(ctx, &k, &k_scale_vrf, &k_weight_vrf, &k_rms_vrf, 2, &mut qk);
