...
pub fn sliding_project_qkv_qb(
    ...
    // one cluster only (V155).
    let x8: DmTensor<f8e4m3, Chip, m![Qs / 2048], m![Dummy8, 1 # 32], m![Dummy2, H]> = x2_hbm.to_dm(&mut ctx.tdma);
    // Q2b: the broadcast pass writes the TRF copy of x itself (no DM commit, no Sub staging).
    ...
    // q, k and v come back one head per slice; the head-wise RMSNorms and RoPE stay in that
    // layout (no transposes in or broadcasts out) and the outputs are written from it.
    let q = sliding::projection::project_query_trf_on(ctx, &x_trf, &q_weight);
    let (k, v) = sliding::projection::project_key_value_trf_on(ctx, &x_trf, &k_weight, &v_weight);
...
/// V247: the three head-RMSNorm sqrt passes moved to SubContext.
/// V256: the K/V projections as a chunked reduction (the qkv structure probe).
#[device(chip = 1)]
pub fn sliding_project_qkv_chunked(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    let x: DmTensor<f8e4m3, Chip, m![Qs / 2048], m![Dummy8, Dummy256 / 8], m![Dummy2, H]> = ctx
        ...
        .commit();
    let x: DmTensor<f8e4m3, Chip, layout::BothClusters, Replicated, m![Dummy2, H]> = unsafe { x.reshape() };
    let k_weight = sliding::projection::load_kv_weight_chunked(ctx, k_weight);
    let v_weight = sliding::projection::load_kv_weight_chunked(ctx, v_weight);

    // q, k and v come back one head per slice; the head-wise RMSNorms and RoPE stay in that
    // layout (no transposes in or broadcasts out) and the outputs are written from it.
    let q = sliding::projection::project_query(ctx, &x, &q_weight);
    let (k, v) = sliding::projection::project_key_value_chunked(ctx, &x2_hbm, &k_weight, &v_weight);
...
/// V247: the three head-RMSNorm sqrt passes moved to SubContext.
#[device(chip = 1)]
pub fn sliding_project_qkv_sub(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // The projections' per-channel weight scales are folded into the head RMSNorms (their
    // loads are eight descriptors in the head layout instead of 512 in the projection layout).
    let q = sliding::rmsnorm::normalize_query_heads_sub::<layout::HeadClusters, layout::HeadSlicesPerCluster>(
        ctx,
        &q,
        q_weight_scale,
        q_rms_weight,
    );
    let k = sliding::rmsnorm::normalize_key_heads_sub::<layout::HeadClusters, layout::HeadSlicesPerCluster>(
        ctx,
        &k,
        k_weight_scale,
        k_rms_weight,
    );
    let v = sliding::rmsnorm::normalize_value_heads_sub::<layout::HeadClusters, layout::HeadSlicesPerCluster>(
        ctx,
        &v,
        v_weight_scale,
    );
...
/// V244 gating probe: the channel scale folded into the ring-64 gather.
#[device(chip = 1)]
pub fn sliding_project_qkv_probe(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // q, k and v come back one head per slice; the head-wise RMSNorms and RoPE stay in that
    // layout (no transposes in or broadcasts out) and the outputs are written from it.
    let q = sliding::projection::project_query_gather_scaled(ctx, &x, &q_weight, q_weight_scale);
    let (k, v) = sliding::projection::project_key_value(ctx, &x, &k_weight, &v_weight);
...
/// V257: attn_out with a single f8 piece of x.
#[device(chip = 1)]
pub fn sliding_attention_output_one_piece(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
...
/// E5c/V269: attn_out with O-weight tiles 92/28.
#[device(chip = 1)]
pub fn sliding_attention_output_e5_92(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    // each slice's Qs chunk straight from there instead of broadcasting x through the switch.
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let x_hbm = sliding::projection::project_output_e5_92(ctx, x, o_weight);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
/// E5c/V269: attn_out with O-weight tiles 100/20.
#[device(chip = 1)]
pub fn sliding_attention_output_e5_100(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    // each slice's Qs chunk straight from there instead of broadcasting x through the switch.
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let x_hbm = sliding::projection::project_output_e5_100(ctx, x, o_weight);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
/// X1/V271: 96/24 tiles, contraction output stored at 256 B-aligned offsets.
#[device(chip = 1)]
pub fn sliding_attention_output_e5_96_x1(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
...
pub fn sliding_attention_output_direct(
    ...
    // each slice's Qs chunk straight from there instead of broadcasting x through the switch.
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let x_hbm = sliding::projection::project_output_direct(ctx, x, o_weight);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
/// V248: the attention-output contraction with a 32-element fetch packet.
#[device(chip = 1)]
pub fn sliding_attention_output_p32(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    // each slice's Qs chunk straight from there instead of broadcasting x through the switch.
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let x_hbm = sliding::projection::project_output_p32(ctx, x, o_weight);
    // Both operands of the post-attention RMSNorm are loaded straight into its reducing layout.
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    // The result is stored straight from the reducing layout (eight descriptors, no switch pass).
...
/// V247: the attention-output epilogue loads hoisted ahead of the weight stream.
#[device(chip = 1)]
pub fn sliding_attention_output_hoist(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![Ns, Gs, Ds]>,
    post_attn_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    o_weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    ...
    // The attention output already lives in HBM as [Ns, Gs, Ds] = [Qs]; project_output loads
    // each slice's Qs chunk straight from there instead of broadcasting x through the switch.
    // V247: the epilogue's three small [H] loads are issued before the O-weight stream instead of
    // inside it (see `normalize_add_scaled_reduced_pre`).
    let scale_dm = shared::rmsnorm::load_reducing::<Cluster>(ctx, o_weight_scale);
    let weight_dm = shared::rmsnorm::load_reducing::<Cluster>(ctx, post_attn_rms_weight);
    let residual = shared::rmsnorm::load_reducing::<Cluster>(ctx, residual_hbm);
    let x: HbmTensorView<'_, bf16, Chip, m![Qs]> = unsafe { x.view().reshape() };
    let x_hbm = sliding::projection::project_output(ctx, x, o_weight);
    let x = shared::rmsnorm::load_reducing::<Cluster>(ctx, &x_hbm);
    let residual = shared::rmsnorm::normalize_add_scaled_reduced_pre::<Cluster>(ctx, &x, &scale_dm, &weight_dm, &residual);
    residual.view().to_hbm_view(&mut ctx.tdma, residual_hbm.view_mut());
}
...
/// V246: the down projection on 30 rows x four 3,840-column chunks, all 256 slices live.
#[device(chip = 1)]
pub fn decoder_feedforward_v246(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    // The up/gate stage runs on whole rows (V181): each slice's f4 rows and block scales are one
    // contiguous HBM segment each; a segmented load costs twice per byte on hardware (V174).
    let x = shared::mlp::feedforward_v246(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
...
pub fn decoder_feedforward_s1(
    ...
    // The up/gate stage runs on whole rows (V181): each slice's f4 rows and block scales are one
    // contiguous HBM segment each; a segmented load costs twice per byte on hardware (V174).
    let x = shared::mlp::feedforward_s1(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
...
/// V241 sweep: the down pass A in two 30-row tiles.
#[device(chip = 1)]
pub fn decoder_feedforward_v241(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    // The up/gate stage runs on whole rows (V181): each slice's f4 rows and block scales are one
    // contiguous HBM segment each; a segmented load costs twice per byte on hardware (V174).
    let x = shared::mlp::feedforward_v241(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
