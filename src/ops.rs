...
/// V208 probe p0: one extra 15.73 MB query-weight load, qkv's own 30,720-byte runs.
#[device(chip = 1)]
pub fn sliding_project_qkv_p0(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
...
/// V208 probe p1: one extra 15.73 MB query-weight load, the same bytes in 1,920-byte runs.
#[device(chip = 1)]
