...
/// V356 harness kernel (arm cf): production ffn with the tail multiply and the post-FF norm's final pass committing bf16
/// through `commit_cast` (V352 c on ffn).
#[device(chip = 1)]
pub fn decoder_feedforward_cf(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    let erf_all = shared::xsw::broadcast_scalar_blocks(ctx, erf_b);
    let out_tail: DmTensor<f32, Chip, Cluster, shared::rmsnorm::ReducingSlices, m![1 # 8]> = unsafe { out_b.reshape() };
    let x = shared::mlp::feedforward_fo_cc(
        ctx,
        x_rep,
        erf_all,
        out_tail,
    ...
    let residual: DmTensor<bf16, Chip, Cluster, shared::rmsnorm::ReducingSlices, m![H % 480]> = unsafe { residual_b.reshape() };
    let residual = shared::rmsnorm::normalize_add_gate_reduced_cc::<Cluster>(ctx, &x, post_ff_rms_weight, &residual, layer_scalar);
    residual.view().to_hbm_view(&mut ctx.tdma, residual_hbm.view_mut());
}

/// V356 harness kernel (arm ccf): cf on top of V354 cc (the fused pre-FF norm commits bf16, no cast pass before the split).
#[device(chip = 1)]
pub fn decoder_feedforward_ccf(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
