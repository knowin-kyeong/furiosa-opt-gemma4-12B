...
/// V227 measurement variant: up/gate contracted against stationary weights (Lane = 8 rows).
#[device(chip = 1)]
pub fn decoder_feedforward_v227(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    // The up/gate stage runs on whole rows (V181): each slice's f4 rows and block scales are one
    // contiguous HBM segment each; a segmented load costs twice per byte on hardware (V174).
    let x = shared::mlp::feedforward_v227(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
...
/// V227 measurement variant with the TRF fills on the main context.
#[device(chip = 1)]
pub fn decoder_feedforward_v227m(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    // The up/gate stage runs on whole rows (V181): each slice's f4 rows and block scales are one
    // contiguous HBM segment each; a segmented load costs twice per byte on hardware (V174).
    let x = shared::mlp::feedforward_v227m(
        ctx,
        &x2_hbm,
        &erf_scale,
        &out_scale,
