...
/// V310 probe arm s8: production ffn plus one extra down_weight_scale load (probe_down_scale_8).
#[device(chip = 1)]
pub fn decoder_feedforward_s8(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    shared::mlp::probe_down_scale_8(ctx, down_weight_scale);
    // The residual is loaded once, straight into the RMSNorm reducing layout, and serves both
    // the pre-FF normalization and the final residual add.
    // V293: the pre-FF norm, the hi/lo split and the geglu scalars run on both clusters (8 copies x 8 chunks in
    // every 32-slice sub-ring) and x is replicated on chip: no x2 HBM hop, so no ExplicitSync idles the DMA queue
...
/// V310 probe arm s4: production ffn plus one extra down_weight_scale load (probe_down_scale_4).
#[device(chip = 1)]
pub fn decoder_feedforward_s4(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    shared::mlp::probe_down_scale_4(ctx, down_weight_scale);
    // The residual is loaded once, straight into the RMSNorm reducing layout, and serves both
    // the pre-FF normalization and the final residual add.
    // V293: the pre-FF norm, the hi/lo split and the geglu scalars run on both clusters (8 copies x 8 chunks in
    // every 32-slice sub-ring) and x is replicated on chip: no x2 HBM hop, so no ExplicitSync idles the DMA queue
...
/// V310 probe arm s2: production ffn plus one extra down_weight_scale load (probe_down_scale_2).
#[device(chip = 1)]
pub fn decoder_feedforward_s2(
    ...
    shared::mlp::probe_down_scale_2(ctx, down_weight_scale);
    // The residual is loaded once, straight into the RMSNorm reducing layout, and serves both
    // the pre-FF normalization and the final residual add.
    // V293: the pre-FF norm, the hi/lo split and the geglu scalars run on both clusters (8 copies x 8 chunks in
    // every 32-slice sub-ring) and x is replicated on chip: no x2 HBM hop, so no ExplicitSync idles the DMA queue
