...
/// V369 arm `n1`: `sliding_project_qkv` with the input norm reduced to x * input_rms_weight (the rms scalar cancels in
/// the q/k/v head norms; shared::xsw::weight_blocks_bf16) and the bf16 hi/lo staging without a leading cast pass.
#[device(chip = 1)]
pub fn sliding_project_qkv_n1(
    ctx: &mut Context,
    x: &HbmTensor<bf16, Chip, m![H]>,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
