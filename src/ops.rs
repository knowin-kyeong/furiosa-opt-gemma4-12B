...
/// V315 harness kernel l4: V313_submit ffn with the down x pieces in Lane (feedforward_vl4).
#[device(chip = 1)]
pub fn decoder_feedforward_l4(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    // The up/gate stage runs on whole rows (V181): each slice's f4 rows and block scales are one
    // contiguous HBM segment each; a segmented load costs twice per byte on hardware (V174).
    let x = shared::mlp::feedforward_vl4(
        ctx,
        x_rep,
        erf_all,
        out_tail,
...
/// V315 harness kernel l1: V313_submit ffn with the down x pieces in Lane (feedforward_vl1).
#[device(chip = 1)]
pub fn decoder_feedforward_l1(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    // The up/gate stage runs on whole rows (V181): each slice's f4 rows and block scales are one
    // contiguous HBM segment each; a segmented load costs twice per byte on hardware (V174).
    let x = shared::mlp::feedforward_vl1(
        ctx,
        x_rep,
        erf_all,
        out_tail,
...
/// V331 harness kernel iv: V313_submit ffn, inv_s replicated on 64 slices/cluster + ring-4 broadcast.
#[device(chip = 1)]
pub fn decoder_feedforward_iv(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    // The up/gate stage runs on whole rows (V181): each slice's f4 rows and block scales are one
    // contiguous HBM segment each; a segmented load costs twice per byte on hardware (V174).
    let x = shared::mlp::feedforward_vtp_iv(
        ctx,
        x_rep,
        erf_all,
        out_tail,
...
/// V331 harness kernel i0: V313_submit ffn, inv_s replicated on all 256 slices/cluster (no switch).
#[device(chip = 1)]
pub fn decoder_feedforward_i0(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    // The up/gate stage runs on whole rows (V181): each slice's f4 rows and block scales are one
    // contiguous HBM segment each; a segmented load costs twice per byte on hardware (V174).
    let x = shared::mlp::feedforward_vtp_i0(
        ctx,
        x_rep,
        erf_all,
        out_tail,
...
/// V333 harness kernel cm: V313_submit ffn, bf16 cast inside the last pre-FF RMSNorm pass.
#[device(chip = 1)]
pub fn decoder_feedforward_cm(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
...
/// V348 harness kernel f1u: V313_submit ffn with the f8 pieces of x summed inside pass A (shared::mlp::feedforward_f1u).
#[device(chip = 1)]
pub fn decoder_feedforward_f1u(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    let erf_all = shared::xsw::broadcast_scalar_blocks(ctx, erf_b);
    let out_tail: DmTensor<f32, Chip, Cluster, shared::rmsnorm::ReducingSlices, m![1 # 8]> = unsafe { out_b.reshape() };
    let x = shared::mlp::feedforward_f1u(
        ctx,
        x_rep,
        erf_all,
        out_tail,
...
/// V348 harness kernel f1d: V313_submit ffn with the f8 pieces of x summed inside pass A (shared::mlp::feedforward_f1d).
#[device(chip = 1)]
pub fn decoder_feedforward_f1d(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
...
/// V348 harness kernel f1ud: V313_submit ffn with the f8 pieces of x summed inside pass A (shared::mlp::feedforward_f1ud).
#[device(chip = 1)]
pub fn decoder_feedforward_f1ud(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
    ...
    let erf_all = shared::xsw::broadcast_scalar_blocks(ctx, erf_b);
    let out_tail: DmTensor<f32, Chip, Cluster, shared::rmsnorm::ReducingSlices, m![1 # 8]> = unsafe { out_b.reshape() };
    let x = shared::mlp::feedforward_f1ud(
        ctx,
        x_rep,
        erf_all,
        out_tail,
...
/// V350 harness kernel fo: V348 f1d with the geglu pieces staged by one store (shared::mlp::feedforward_fo).
#[device(chip = 1)]
pub fn decoder_feedforward_fo(
    ctx: &mut Context,
    residual_hbm: &mut HbmTensor<bf16, Chip, m![H]>,
    pre_ff_rms_weight: &HbmTensor<bf16, Chip, m![H]>,
    up_weight_packed: &HbmTensor<f4e2m1, Chip, m![L, H]>,
